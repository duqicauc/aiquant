...
class TestBackupCacheManager:
    ...
    def temp_db_path(self, temp_dir):
        """临时数据库路径"""
        return str(temp_dir / "test_backup_cache.db")
    ...
    def test_init_with_backup(self, temp_db_path, temp_backup_dir):
        """测试初始化时启用备份"""
        manager = BackupCacheManager(db_path=temp_db_path, backup_dir=temp_backup_dir, enable_backup=True)
    ...
    def test_init_without_backup(self, temp_db_path):
        """测试初始化时不启用备份"""
        manager = BackupCacheManager(db_path=temp_db_path, enable_backup=False)
    ...
    def test_save_data_with_backup(self, temp_db_path, temp_backup_dir, sample_data):
        ...
        manager.save_data_with_backup(df=sample_data, data_type="daily_data", ts_code="000001.SZ")
    ...
    def test_get_data_with_backup_from_csv(self, temp_db_path, temp_backup_dir, sample_data):
        ...
        data = manager.get_data_with_backup(
            ts_code="000001.SZ", data_type="daily_data", start_date="20240101", end_date="20241231"
        )
    ...
    def test_save_to_csv_new_file(self, temp_backup_dir, sample_data):
        """测试保存新CSV文件"""
        manager = BackupCacheManager(backup_dir=temp_backup_dir, enable_backup=True)
    ...
    def test_read_from_csv_exists(self, temp_backup_dir, sample_data):
        ...
        data = manager.read_from_csv(
            ts_code="000001.SZ", data_type="daily_data", start_date=first_date, end_date=last_date
        )
    ...
    def test_read_from_csv_not_exists(self, temp_backup_dir):
        ...
        data = manager.read_from_csv(
            ts_code="999999.SZ", data_type="daily_data", start_date="20240101", end_date="20241231"
        )
