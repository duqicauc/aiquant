...
# 页面配置
st.set_page_config(page_title="AIQuant 量化分析平台", page_icon="📈", layout="wide", initial_sidebar_state="expanded")
...
if page == "🏠 市场概况":
            ...
            with st.spinner(f"正在诊断 {stock_code}，请稍候..."):
                ...
                if "error" in report:
                    ...
                    with header_col1:
                        ...
                        st.markdown(
                            f"**行业**: {basic.get('industry', 'N/A')} | **最新价**: ¥{basic.get('latest_price', 0):.2f}"
                        )
                    ...
                    if report_html:
                        ...
                        st.markdown("💡 **提示**: 下载HTML报告后在浏览器中打开，可查看交互式图表")
                    ...
                    with tab5:
                        ...
                        if trading_plan:
                            ...
                            with col2:
                                ...
                                st.markdown(
                                    f"**止损价**: ¥{exit_plan.get('stop_loss', 0):.2f} ({exit_plan.get('stop_loss_pct', 0):.1f}%)"
                                )
                                st.markdown(
                                    f"**第一目标**: ¥{exit_plan.get('take_profit_1', 0):.2f} (+{exit_plan.get('take_profit_1_pct', 0):.1f}%)"
                                )
                                st.markdown(
                                    f"**第二目标**: ¥{exit_plan.get('take_profit_2', 0):.2f} (+{exit_plan.get('take_profit_2_pct', 0):.1f}%)"
                                )
                                st.markdown(
                                    f"**第三目标**: ¥{exit_plan.get('take_profit_3', 0):.2f} (+{exit_plan.get('take_profit_3_pct', 0):.1f}%)"
                                )
    ...
    # 文件上传
    uploaded_file = st.file_uploader("上传股票列表文件", type=["csv", "xlsx", "xls"], help="支持 CSV 和 Excel 格式")

    # 或者手动输入
    st.markdown("**或者** 手动输入股票代码（每行一个）：")

    manual_codes = st.text_area("股票代码列表", placeholder="000001.SZ\n600519.SH\n300750.SZ", height=150)
        ...
        if stock_codes:
            ...
            for i, code in enumerate(stock_codes):
                ...
                try:
                    ...
                    results.append(
                        {"股票代码": code, "股票名称": "N/A", "综合评分": 0, "操作建议": f"错误: {str(e)[:30]}"}
                    )
            ...
            if results:
                ...
                with col2:
                    buy_count = (
                        len(df_results[df_results["操作建议"] == "买入"]) if "操作建议" in df_results.columns else 0
                    )
    ...
    if not df_pred.empty:
        ...
        st.download_button(
            "📥 下载预测结果", csv, file_name=f"prediction_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv"
        )
