...
def evaluate_predictions(dm, predictions_file, predict_date, eval_date, version):
    ...
    for _, row in df_eval.iterrows():
        ...
        log.info(f"  {row['ts_code']:<12} {row['name']:<10} {pct_chg:>+10.2f}%  {pre_ret:>+8.1f}%  {actual_ret:>+8.1f}%")
...
def main():
    ...
    if v232_result and v240_result:
        ...
        log.info(f"{'T1前平均涨幅':<20} {v232_result['avg_pre_t1']:>+10.1f}%     {v240_result['avg_pre_t1']:>+10.1f}%     {pre_t1_diff:>+10.1f}%")
        
        # 预测日涨幅对比
        pct_chg_diff = v240_result['avg_pct_chg'] - v232_result['avg_pct_chg']
        log.info(f"{'预测日平均涨幅':<20} {v232_result['avg_pct_chg']:>+10.1f}%     {v240_result['avg_pct_chg']:>+10.1f}%     {pct_chg_diff:>+10.1f}%")
        
        # 追高数量对比
        chase_diff = v240_result['chase_high_count'] - v232_result['chase_high_count']
        log.info(f"{'追高数量(>9%)':<20} {v232_result['chase_high_count']:>10}/10     {v240_result['chase_high_count']:>10}/10     {chase_diff:>+10}")
        
        # 实际收益对比
        return_diff = v240_result['avg_return'] - v232_result['avg_return']
        log.info(f"{'实际平均收益':<20} {v232_result['avg_return']:>+10.1f}%     {v240_result['avg_return']:>+10.1f}%     {return_diff:>+10.1f}%")
        
        # 胜率对比
        win_diff = v240_result['win_rate'] - v232_result['win_rate']
        log.info(f"{'胜率':<20} {v232_result['win_rate']:>10.0f}%     {v240_result['win_rate']:>10.0f}%     {win_diff:>+10.0f}%")
