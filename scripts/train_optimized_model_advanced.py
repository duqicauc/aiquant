...
def extract_features_with_time(df):
    ...
    # 识别数值型特征列（排除元数据列）
    meta_cols = ["sample_id", "unique_sample_id", "trade_date", "ts_code", "name", "label", "sample_type", "days_to_t1"]
...
def timeseries_split(df_features, train_ratio=0.8):
    ...
    feature_cols = [
        col for col in df_features.columns if col not in ["sample_id", "label", "t1_date", "ts_code", "name"]
    ]
...
def analyze_probability_distribution(y_test, y_prob):
    ...
    df_analysis = pd.DataFrame({"label": y_test.values, "probability": y_prob})
