...
def get_market_data(dm, predict_date, lookback_days=120):
    ...
    try:
        ...
        market_df['market_position_20d'] = (market_df['close'] - market_low_20d) / (market_high_20d - market_low_20d + 1e-8)
...
def extract_features_v260(df, market_df=None):
    ...
    if market_df is not None and len(market_df) > 0 and 'trade_date' in df.columns and 'trade_date' in market_df.columns:
        # 转换日期格式
    ...
    df['breakout_volume_strength'] = np.where(
        breakout_20d == 1,
        df['vol'] / (vol_ma20 + 1e-8),
        0
    )
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for idx, row in stocks.iterrows():
            future = executor.submit(
                ...
                version, booster, feature_names, calibrator, market_df
            )
...
def compare_top10(df_v230, df_v260):
    ...
    if len(top10_v230) > 0 and len(top10_v260) > 0:
        ...
        # 综合评分
        score_v230 = (
            mean_v230 * 0.4 +  # 平均收益权重40%
            median_v230 * 0.2 +  # 中位数收益权重20%
            positive_v230 * 5 +  # 正收益数权重（每只5分）
            high_return_v230 * 10  # 高收益数权重（每只10分）
        )
        ...
        log.info(f"  提升: {score_v260 - score_v230:.2f} 分 ({((score_v260 - score_v230) / abs(score_v230) * 100) if score_v230 != 0 else 0:.1f}%)")
        
        if score_v260 > score_v230:
            log.success(f"✅ v2.6.0 表现优于 v2.3.0")
        elif score_v260 < score_v230:
            log.warning(f"⚠️  v2.6.0 表现不如 v2.3.0")
        else:
            log.info(f"➡️  v2.6.0 与 v2.3.0 表现相当")
...
def main():
    ...
    df_predictions_v230 = predict_stocks(
        dm, stocks, args.predict_date, 'v2.3.0',
        booster_v230, feature_names_v230, calibrator_v230, top_n=50
    )
    
    log.info("\n[步骤6] 使用v2.6.0模型预测...")
    df_predictions_v260 = predict_stocks(
        dm, stocks, args.predict_date, 'v2.6.0',
        booster_v260, feature_names_v260, calibrator_v260, top_n=50, market_df=market_df
    )
    
    # 评估实际收益
    log.info("\n[步骤7] 评估实际收益...")
    df_results_v230 = evaluate_predictions(
        df_predictions_v230, dm, args.predict_date, args.evaluate_date, 'v2.3.0'
    )
    df_results_v260 = evaluate_predictions(
        df_predictions_v260, dm, args.predict_date, args.evaluate_date, 'v2.6.0'
    )
