...
def compare_models(df_v251, df_v252):
    """对比两个模型"""
    log.info(f"\n{'='*80}")
    log.info("v2.5.1 vs v2.5.2 对比分析")
    log.info(f"{'='*80}")
    
    # 合并数据
    df_merge = pd.merge(
        df_v251[['feature', 'importance', 'percentage', 'category']].rename(columns={
            'importance': 'importance_v251',
            'percentage': 'percentage_v251',
            'category': 'category_v251'
        }),
    ...
    # 统一category（优先使用v2.5.2的分类）
    df_merge['category'] = df_merge['category_v252'].where(
        df_merge['category_v252'] != 0, 
        df_merge['category_v251']
    )
    
    # 计算变化
    df_merge['importance_change'] = df_merge['importance_v252'] - df_merge['importance_v251']
    df_merge['percentage_change'] = df_merge['percentage_v252'] - df_merge['percentage_v251']
    
    # Top 20变化最大的特征
    log.info(f"\n重要性变化最大的Top 20特征:")
    log.info(f"{'特征':<30} {'v2.5.1占比':<12} {'v2.5.2占比':<12} {'变化':<10} {'类别':<15}")
    log.info("-" * 85)
    
    top_changes = df_merge.nlargest(20, 'percentage_change')[['feature', 'percentage_v251', 'percentage_v252', 'percentage_change', 'category']].copy()
    for idx, row in top_changes.iterrows():
        log.info(f"{row['feature']:<30} {row['percentage_v251']:<12.2f}% {row['percentage_v252']:<12.2f}% {row['percentage_change']:<10.2f}% {row['category']:<15}")
    
    # 按类别对比
    log.info(f"\n按类别对比（占比变化）:")
    log.info(f"{'类别':<20} {'v2.5.1占比':<12} {'v2.5.2占比':<12} {'变化':<10}")
    log.info("-" * 60)
    ...
    all_cats = set(cat_v251.index) | set(cat_v252.index)
    for cat in sorted(all_cats, key=lambda x: cat_v252.get(x, 0), reverse=True):
        pct_v251 = cat_v251.get(cat, 0)
        pct_v252 = cat_v252.get(cat, 0)
        change = pct_v252 - pct_v251
        log.info(f"{cat:<20} {pct_v251:<12.2f}% {pct_v252:<12.2f}% {change:<10.2f}%")
    
    # 检查days_to_t1是否被移除
    if 'days_to_t1' in df_v251['feature'].values:
        days_to_t1_v251 = df_v251[df_v251['feature'] == 'days_to_t1']
        if len(days_to_t1_v251) > 0:
            log.info(f"\n⚠️  v2.5.1中days_to_t1特征:")
            log.info(f"  重要性: {days_to_t1_v251.iloc[0]['importance']:.2f}")
            log.info(f"  占比: {days_to_t1_v251.iloc[0]['percentage']:.2f}%")
    
    if 'days_to_t1' in df_v252['feature'].values:
        log.warning("⚠️  v2.5.2中仍然包含days_to_t1特征！")
    else:
        log.success("✓ v2.5.2中已成功移除days_to_t1特征")
...
def main():
    ...
    # 对比
    df_compare = compare_models(df_imp_v251, df_imp_v252)
    
    # 保存结果
    output_dir = PROJECT_ROOT / 'data' / 'models' / 'breakout_launch_scorer' / 'versions' / 'v2.5.2'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    df_imp_v252.to_csv(output_dir / 'feature_importance_v252.csv', index=False)
