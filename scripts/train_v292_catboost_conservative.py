...
VERSION = "v2.9.2-catboost-conservative"
...
def train_catboost(X_train, y_train, X_val, y_val):
    ...
    model = CatBoostClassifier(
        iterations=500,  # 保守: 800 → 500（减少过拟合）
        depth=6,  # 保守: 8 → 6（减少模型复杂度）
        learning_rate=0.05,  # 保守: 0.08 → 0.05（更保守的学习）
        l2_leaf_reg=5.0,  # 保守: 1.0 → 5.0（增强正则化）
        border_count=128,
...
def save_model(model, feature_names, calibrator, metrics, hard_ratio):
    ...
    metadata = {
        ...
        "description": "v2.9.2-catboost-conservative - 保守超参(depth=6,iter=500,lr=0.05,l2=5.0)+Platt校准",
        "improvements": [
