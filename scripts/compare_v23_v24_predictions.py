...
def compare_top10(df_v23, df_v24):
    ...
    if len(top10_v23) > 0 and len(top10_v24) > 0:
        ...
        score_v23 = (
            mean_v23 * 0.4
            + median_v23 * 0.2  # 平均收益权重40%
            + positive_v23 * 5  # 中位数收益权重20%
            + high_return_v23 * 10  # 正收益数权重（每只5分）  # 高收益数权重（每只10分）
        )

        score_v24 = mean_v24 * 0.4 + median_v24 * 0.2 + positive_v24 * 5 + high_return_v24 * 10

        log.info("\n【综合评分】")
        log.info(f"  v2.3.0: {score_v23:.2f} 分")
        log.info(f"  v2.4.0: {score_v24:.2f} 分")
        log.info(
            f"  提升: {score_v24 - score_v23:.2f} 分 ({((score_v24 - score_v23) / abs(score_v23) * 100) if score_v23 != 0 else 0:.1f}%)"
        )

        if score_v24 > score_v23:
            log.success("✅ v2.4.0 表现优于 v2.3.0")
        elif score_v24 < score_v23:
            log.warning("⚠️  v2.4.0 表现不如 v2.3.0")
        else:
            log.info("➡️  v2.4.0 与 v2.3.0 表现相当")
...
def main():
    ...
    df_predictions_v23 = predict_stocks(
        booster_v23, calibrator_v23, feature_names_v23, dm, stocks, args.predict_date, top_n=50
    )

    log.info("\n[步骤5] 使用v2.4.0模型预测...")
    df_predictions_v24 = predict_stocks(
        booster_v24, calibrator_v24, feature_names_v24, dm, stocks, args.predict_date, top_n=50
    )

    # 评估实际收益
    log.info("\n[步骤6] 评估实际收益...")
    df_results_v23 = evaluate_predictions(df_predictions_v23, dm, args.predict_date, args.evaluate_date, v23_version)
    df_results_v24 = evaluate_predictions(df_predictions_v24, dm, args.predict_date, args.evaluate_date, "v2.4.0")
    ...
    if df_results_v23 is not None:
        output_file_v23 = (
            output_dir / f"{v23_version}_predictions_{args.predict_date}_evaluated_{args.evaluate_date}.csv"
        )
