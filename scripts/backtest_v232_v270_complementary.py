...
def get_stock_close_and_low(date: str, ts_code: str, dm: DataManager) -> Tuple[Optional[float], Optional[float]]:
    ...
    try:
        ...
        close = (
            float(row["close"])
            if "close" in df_daily.columns and pd.notna(row.get("close")) and row["close"] > 0
            else None
        )
...
def backtest_complementary_strategy(
    ...
    holdings: Dict[str, Dict] = (
        {}
    )  # {ts_code: {'quantity': int, 'cost': float, 'buy_date': str, 'below_ma5_days': int}}
    total_buy_fees = 0.0
    total_sell_fees = 0.0
    ...
    for i, date in enumerate(trading_dates):
        ...
        if exclude_sectors:
            try:
                ...
                mask = df_pred["industry"].apply(lambda x: not any(s in str(x) for s in EXCLUDED_SECTORS_FOR_BUY))
                ...
                if excluded_count > 0:
                    log.info(
                        f"排除银行/证券/白酒/房地产后: 池内{len(df_pred)}只 -> {len(df_filtered)}只（排除{excluded_count}只）"
                    )
            except Exception as e:
        ...
        for ts_code in stocks_to_buy:
            ...
            if enable_execution_constraints:
                ...
                if (
                    not np.isnan(pre_close)
                    and pre_close > 0
                    and is_open_limit_up(raw_open, float(pre_close), limit_pct)
                ):
            ...
            buy_fee = (
                calc_trade_fee(gross, False, commission_rate, min_commission, transfer_fee_rate, stamp_tax_rate)
                if enable_fees
                else 0.0
            )
            ...
            if enable_execution_constraints:
                ...
                if not np.isnan(amt_thousand) and float(amt_thousand) > 0:
                    ...
                    if not ok_part:
                        blocked_reasons[REASON_VOLUME_INSUFFICIENT] = (
                            blocked_reasons.get(REASON_VOLUME_INSUFFICIENT, 0) + 1
                        )
                ...
                if stop_loss_mode == "close":
                    ...
                    if low_price is not None and low_price <= stop_price:
                        stocks_to_sell.append(
                            (ts_code, stop_price, None, f"单标亏损达{stop_loss_pct:.0f}%(日内最低触及)")
                        )
                        continue
        ...
        for sell_info in stocks_to_sell:
            ...
            sell_fee = (
                calc_trade_fee(sell_gross, True, commission_rate, min_commission, transfer_fee_rate, stamp_tax_rate)
                if enable_fees
                else 0.0
            )
...
def _plot_equity_curve(
    result: Dict, output_dir: Path, df_curve: pd.DataFrame, csi300_curve: Optional[pd.DataFrame]
) -> Optional[str]:
...
def generate_report(result: Dict, output_dir: Path):
    ...
    try:
        ...
        codes_from_ops = result["operations_log"]["ts_code"].tolist() if not result["operations_log"].empty else []
    ...
    daily_file = (
        output_dir
        / f"backtest_daily_{result['start_date']}_{result['end_date']}_sl_{sl_tag}{file_suffix}{friction_tag}.csv"
    )
    ...
    if not result["operations_log"].empty:
        operations_file = (
            output_dir
            / f"backtest_operations_{result['start_date']}_{result['end_date']}_sl_{sl_tag}{file_suffix}{friction_tag}.csv"
        )
    ...
    report_file = (
        output_dir
        / f"backtest_report_{result['start_date']}_{result['end_date']}_sl_{sl_tag}{file_suffix}{friction_tag}.md"
    )
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        if not df_sell.empty:
            if "buy_date" not in df_sell.columns or df_sell["buy_date"].isna().all():
                ...
                def _infer_buy_date(row):
                    sub = df_buy[
                        (df_buy["ts_code"] == row["ts_code"]) & (df_buy["date"].astype(str) < str(row["date"]))
                    ]
        ...
        sl_labels = {
            "none": "无4%硬止损",
            "close": f"{sl_pct:.0f}%止损(收盘价触发)",
            "intraday_low": f"{sl_pct:.0f}%止损(日内最低价触及)",
        }
        ...
        if result.get("exclude_sectors"):
            f.write(
                f"- 买入策略: 从互补策略Top{SECTOR_FILTER_POOL_N}中排除银行、证券、白酒、房地产板块后取Top{result.get('top_n_buy', 10)}\n"
            )
        ...
        f.write(
            f"- 执行口径: {'含滑点+费用+涨跌停/停牌/量能约束（贴近实盘）' if result.get('apply_frictions', True) else '理想成交（无摩擦，用于对比）'}\n\n"
        )
        ...
        if not df_sell.empty:
            f.write("## 平均持仓时间与盈亏 Top5\n\n")
            f.write(
                f"- **平均持仓时间**：{f'{avg_hold_days:.1f} 天' if avg_hold_days is not None else '—（无买入日期记录）'}\n\n"
            )
            # 最大收益 Top5 股票
            top5_profit = df_sell.nlargest(5, "profit")
            if not top5_profit.empty:
                f.write("### 最大收益 Top5 股票\n\n")
                f.write("| 股票代码 | 股票名称 | 板块 | 买入日期 | 卖出日期 | 持仓天数 | 盈亏(元) | 盈亏% |\n")
                f.write("|----------|----------|------|----------|----------|----------|----------|-------|\n")
                for _, row in top5_profit.iterrows():
                    ...
                    if pd.notna(bd) and str(bd) != "—" and str(bd) != "nan":
                        days = (
                            pd.to_datetime(str(sd), format="%Y%m%d") - pd.to_datetime(str(bd), format="%Y%m%d")
                        ).days
                    else:
                        days = "—"
                    f.write(
                        f"| {row['ts_code']} | {_name(row['ts_code'])} | {_sector(row['ts_code'])} | {bd} | {sd} | {days} | {row['profit']:+,.0f} | {row['profit_pct']:+.2f}% |\n"
                    )
                f.write("\n")
            # 最大亏损 Top5 股票
            top5_loss = df_sell.nsmallest(5, "profit")
            if not top5_loss.empty:
                f.write("### 最大亏损 Top5 股票\n\n")
                f.write("| 股票代码 | 股票名称 | 板块 | 买入日期 | 卖出日期 | 持仓天数 | 盈亏(元) | 盈亏% |\n")
                f.write("|----------|----------|------|----------|----------|----------|----------|-------|\n")
        ...
        f.write("## 最终持仓\n\n")
        f.write(
            f"持仓数量: {len(result['final_holdings'])}只；以下为买入日期、当前成本、期末价、市值及盈亏状态（期末日: {result['end_date']}）。\n\n"
        )
        if result["final_holdings"]:
            f.write(
                "| 股票代码 | 股票名称 | 板块 | 买入日期 | 持仓数量 | 成本(元) | 当前价(元) | 市值(元) | 盈亏(元) | 盈亏% | 状态 |\n"
            )
            f.write(
                "|---------|----------|------|----------|----------|----------|------------|----------|----------|-------|------|\n"
            )
            for ts_code, position in result["final_holdings"].items():
                ...
                if current_price is not None and current_price > 0:
                    ...
                    f.write(
                        f"| {ts_code} | {_name(ts_code)} | {_sector(ts_code)} | {buy_date} | {qty}股 | {cost:,.0f} | {current_price:.2f} | {market_value:,.0f} | {profit:+,.0f} | {profit_pct:+.2f}% | {status} |\n"
                    )
                else:
                    f.write(
                        f"| {ts_code} | {_name(ts_code)} | {_sector(ts_code)} | {buy_date} | {qty}股 | {cost:,.0f} | — | — | — | — | — |\n"
                    )
        f.write("\n")
        ...
        if not df_op.empty:
            f.write("## 详细买卖记录\n\n")
            f.write("以下按时间顺序列出所有买入与卖出操作，含价格、金额与原因。\n\n")
            # 买入记录表
            df_buy = df_op[df_op["operation"] == "买入"]
            if not df_buy.empty:
                f.write("### 买入记录\n\n")
                f.write("| 日期 | 股票代码 | 股票名称 | 板块 | 数量 | 价格(元) | 金额(元) | 买卖原因 |\n")
                f.write("|------|----------|----------|------|------|----------|----------|----------|\n")
                for _, row in df_buy.iterrows():
                    f.write(
                        f"| {row['date']} | {row['ts_code']} | {_name(row['ts_code'])} | {_sector(row['ts_code'])} | {row['quantity']} | {row['price']:.2f} | {row['amount']:,.0f} | {row['reason']} |\n"
                    )
                f.write("\n")
            # 卖出记录表
            ...
            if not df_sell.empty:
                f.write("### 卖出记录\n\n")
                f.write(
                    "| 日期 | 股票代码 | 股票名称 | 板块 | 数量 | 卖出价(元) | 金额(元) | 成本(元) | 盈亏(元) | 盈亏% | 卖出原因 |\n"
                )
                f.write(
                    "|------|----------|----------|------|------|------------|----------|----------|----------|-------|----------|\n"
                )
                for _, row in df_sell.iterrows():
                    ...
                    f.write(
                        f"| {row['date']} | {row['ts_code']} | {_name(row['ts_code'])} | {_sector(row['ts_code'])} | {row['quantity']} | {row['price']:.2f} | {row['amount']:,.0f} | {cost:,.0f} | {profit:+,.0f} | {profit_pct:+.2f}% | {row['reason']} |\n"
                    )
                f.write("\n")
            ...
            if not df_sell.empty and "profit" in df_sell.columns and "reason" in df_sell.columns:
                f.write("### 按卖出原因的盈亏汇总\n\n")
                f.write("| 卖出原因 | 笔数 | 盈利笔数 | 亏损笔数 | 总盈亏(元) | 平均盈亏% |\n")
                f.write("|----------|------|----------|----------|------------|----------|\n")
        ...
        if not df_daily.empty:
            ...
            if csi300_curve is not None and not csi300_curve.empty:
                ...
                df_curve["csi300_return_pct"] = np.where(
                    df_curve["nav_csi300"].notna(), df_curve["nav_csi300"] - 100, np.nan
                )
                f.write(
                    "| 日期 | 策略总资产(元) | 策略净值 | 策略累计收益% | 沪深300收盘 | 沪深300净值 | 沪深300累计收益% |\n"
                )
                f.write(
                    "|------|----------------|----------|---------------|--------------|--------------|------------------|\n"
                )
                for _, row in df_curve.iterrows():
                    ...
                    f.write(
                        f"| {row['date']} | {row['total_assets']:,.0f} | {row['nav_strategy']:.2f} | {row['total_return_pct']:+.2f}% | {csi_close} | {csi_nav} | {csi_ret} |\n"
                    )
            else:
                f.write("| 日期 | 策略总资产(元) | 策略净值 | 策略累计收益% |\n")
                f.write("|------|----------------|----------|---------------|\n")
                for _, row in df_curve.iterrows():
                    f.write(
                        f"| {row['date']} | {row['total_assets']:,.0f} | {row['nav_strategy']:.2f} | {row['total_return_pct']:+.2f}% |\n"
                    )
            f.write("\n")
            # 绘制资金曲线图（策略 vs 沪深300）
        ...
        if not df_daily.empty:
            f.write("## 每日资产与买卖摘要\n\n")
            f.write(
                "| 日期 | 现金(元) | 持仓市值(元) | 总资产(元) | 累计收益% | 持仓数 | 当日买入 | 当日卖出 | 买入原因摘要 | 卖出原因摘要 |\n"
            )
            f.write(
                "|------|----------|--------------|------------|------------|--------|----------|----------|--------------|--------------|\n"
            )
            for _, row in df_daily.iterrows():
                f.write(
                    f"| {row['date']} | {row['cash']:,.0f} | {row['position_value']:,.0f} | {row['total_assets']:,.0f} | {row['total_return_pct']:+.2f}% | {row['holdings_count']} | {row['buy_count']} | {row['sell_count']} | {row.get('buy_reason_remark', '')} | {row.get('sell_reason_remark', '')} |\n"
                )
            f.write("\n")
        ...
        f.write("## 分析报告小结\n\n")
        f.write(
            f"1. **收益与风险**：回测区间内收益率 **{result['final_return_pct']:+.2f}%**，最大回撤 **{result['max_drawdown']:.2f}%**（{result.get('max_drawdown_date', '')}）。\n\n"
        )
        f.write(
            f"2. **交易质量**：共买入 {result['total_buys']} 笔、卖出 {result['total_sells']} 笔；卖出胜率 **{result['win_rate']:.2f}%**（盈利 {result['win_trades']} 笔 / 亏损 {result['loss_trades']} 笔），平均每笔盈亏 **{result['avg_profit']:+,.0f} 元**（{result['avg_profit_pct']:+.2f}%）。\n\n"
        )
        pf = result.get("profit_factor", 0)
        plr = result.get("profit_loss_ratio", 0)
        pf_s = (
            f"盈利因子 **{pf:.2f}**、平均盈利/平均亏损 **{plr:.2f}**"
            if pf != float("inf") and plr != float("inf")
            else "盈亏比见上文"
        )
        ...
        if csi300 is not None and not csi300.empty and len(csi300) >= 2:
            ...
            f.write(
                f"4. **与沪深300对比**：策略收益率 **{result['final_return_pct']:+.2f}%**，同期沪深300累计收益 **{csi300_return:+.2f}%**；资金曲线图见上文。\n\n"
            )
        sl_desc_map = {
            "none": "无4%硬止损",
...
def main():
    ...
    parser.add_argument(
        "--no-frictions", action="store_true", help="理想成交：不计滑点/费用/涨跌停与量能阻塞（输出文件名带 _ideal）"
    )
