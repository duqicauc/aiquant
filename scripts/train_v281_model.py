...
# -*- coding: utf-8 -*-
"""
v2.8.1 集成模型训练脚本（使用 Tushare 指标对齐版本）

在 v2.8.0 基础上，训练数据中的技术指标已替换为 Tushare stk_factor_pro 版本，
确保训练和预测使用完全一致的特征来源。

Usage:
    python scripts/train_v281_model.py
"""
...
def load_training_data():
    """加载 Tushare 对齐后的训练数据"""
    log.info("加载训练数据（Tushare 对齐版）...")

    enhanced_dir = PROJECT_ROOT / "data" / "training" / "enhanced_tushare"

    # 如果 Tushare 对齐版本不存在，回退到原始 enhanced
    if not enhanced_dir.exists():
        log.warning("Tushare 对齐版本不存在，使用原始 enhanced 数据")
        enhanced_dir = PROJECT_ROOT / "data" / "training" / "enhanced"

    pos_file = enhanced_dir / "feature_data_34d_v5_enhanced_tushare.csv"
    neg_file = enhanced_dir / "negative_feature_data_v2_34d_v5_enhanced_tushare.csv"
    hard_neg_file = enhanced_dir / "hard_negative_feature_data_34d_v5_enhanced_tushare.csv"
...
def main():
    log.info("=" * 80)
    log.info("v2.8.1 集成模型训练（Tushare 指标对齐版）")
    log.info("=" * 80)
    ...
    y_train = train["label"]
    X_cal = cal[valid_features].fillna(0)
    y_cal = cal["label"]
    X_test = test[valid_features].fillna(0)
    y_test = test["label"]

    # 训练三个模型
    xgb_model, xgb_auc = train_xgboost(X_train, y_train, X_cal, y_cal, valid_features)
    lgb_model, lgb_auc = train_lightgbm(X_train, y_train, X_cal, y_cal, valid_features)
    ...
    # 保存模型
    model_dir = PROJECT_ROOT / "data" / "models" / "breakout_launch_scorer" / "versions" / "v2.8.1-ensemble" / "model"
