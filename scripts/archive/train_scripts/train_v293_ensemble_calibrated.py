...
# -*- coding: utf-8 -*-
"""
v2.9.3 集成模型训练脚本（端到端概率校准版）

在 v2.9.1 基础上引入三层校准体系：
1. LGB best_iteration 持久化
2. 子模型独立 Platt Scaling 校准
3. Stacking 元学习器（逻辑回归）替代简单加权平均

Usage:
    python scripts/train_v293_ensemble_calibrated.py
"""
...
def train_lightgbm(X_train, y_train, X_val, y_val, feature_names, scale_pos_weight):
    ...
    log.info(f"  LightGBM best_iteration: {model.best_iteration}, num_trees: {model.num_trees()}")
...
def main():
    ...
    # ==================== 阶段二：子模型独立概率校准 ====================
    log.info("\n" + "=" * 80)
    log.info("阶段二：子模型独立概率校准 (Platt Scaling)")
    log.info("=" * 80)

    # 在校准集上获取原始概率
    cal_raw_xgb = xgb_model.predict(xgb.DMatrix(X_cal, feature_names=valid_features))
    cal_raw_lgb = lgb_model.predict(X_cal, num_iteration=lgb_model.best_iteration)
    cal_raw_cat = cat_model.predict_proba(X_cal)[:, 1]

    log.info("校准集原始概率分布:")
    log.info(f"  XGB: mean={cal_raw_xgb.mean():.4f}, std={cal_raw_xgb.std():.4f}")
    log.info(f"  LGB: mean={cal_raw_lgb.mean():.4f}, std={cal_raw_lgb.std():.4f}")
    log.info(f"  CAT: mean={cal_raw_cat.mean():.4f}, std={cal_raw_cat.std():.4f}")

    # 训练 Platt Scaling 校准器
    calibrators = {}
    for name, raw_pred in [("xgboost", cal_raw_xgb), ("lightgbm", cal_raw_lgb), ("catboost", cal_raw_cat)]:
        platt = PlattScaler()
        platt.fit(raw_pred, y_cal)
        calibrators[name] = platt
        calibrated = platt.predict(raw_pred)
        log.info(f"  {name} Platt 校准后: mean={calibrated.mean():.4f}, std={calibrated.std():.4f}")

    # ==================== 阶段三：Stacking 元学习器 ====================
    log.info("\n" + "=" * 80)
    log.info("阶段三：Stacking 元学习器")
    log.info("=" * 80)

    # 构建元特征（校准后的概率）
    X_meta = np.column_stack(
        [
            calibrators["xgboost"].predict(cal_raw_xgb),
            calibrators["lightgbm"].predict(cal_raw_lgb),
            calibrators["catboost"].predict(cal_raw_cat),
        ]
    )

    meta_learner = LogisticRegression(C=1.0, solver="lbfgs", max_iter=1000)
    meta_learner.fit(X_meta, y_cal)
    log.info(
        f"元学习器系数: xgb={meta_learner.coef_[0][0]:.4f}, lgb={meta_learner.coef_[0][1]:.4f}, cat={meta_learner.coef_[0][2]:.4f}, intercept={meta_learner.intercept_[0]:.4f}"
    )

    # ==================== 阶段四：测试集对比验证 ====================
    log.info("\n" + "=" * 80)
    log.info("阶段四：测试集对比验证")
    log.info("=" * 80)

    test_raw_xgb = xgb_model.predict(xgb.DMatrix(X_test, feature_names=valid_features))
    test_raw_lgb = lgb_model.predict(X_test, num_iteration=lgb_model.best_iteration)
    test_raw_cat = cat_model.predict_proba(X_test)[:, 1]

    # 旧版加权平均
    old_ensemble = (
        test_raw_xgb * weights["xgboost"] + test_raw_lgb * weights["lightgbm"] + test_raw_cat * weights["catboost"]
    )

    # 新版：校准后 Stacking
    X_test_meta = np.column_stack(
        [
            calibrators["xgboost"].predict(test_raw_xgb),
            calibrators["lightgbm"].predict(test_raw_lgb),
            calibrators["catboost"].predict(test_raw_cat),
        ]
    )
    new_ensemble = meta_learner.predict_proba(X_test_meta)[:, 1]

    log.info("测试集指标对比:")
    log.info(
        f"  [旧] 加权平均  AUC={roc_auc_score(y_test, old_ensemble):.4f}  Brier={brier_score_loss(y_test, old_ensemble):.4f}"
    )
    log.info(
        f"  [新] Stacking  AUC={roc_auc_score(y_test, new_ensemble):.4f}  Brier={brier_score_loss(y_test, new_ensemble):.4f}"
    )

    # 子模型校准前后对比
    for name, raw_pred in [("xgboost", test_raw_xgb), ("lightgbm", test_raw_lgb), ("catboost", test_raw_cat)]:
        cal_pred = calibrators[name].predict(raw_pred)
        log.info(
            f"  {name} 校准前 Brier={brier_score_loss(y_test, raw_pred):.4f}  校准后 Brier={brier_score_loss(y_test, cal_pred):.4f}"
        )

    # ==================== 保存模型与校准器 ====================
    model_version = "v2.9.3-ensemble"
    ...
    for name, calibrator in calibrators.items():
        joblib.dump(calibrator, str(model_dir / f"calibrator_{name}.pkl"))

    # 保存元学习器
    joblib.dump(meta_learner, str(model_dir / "meta_learner.pkl"))
    ...
    # 保存指标
    ensemble_auc_new = roc_auc_score(y_test, new_ensemble)
    ensemble_precision_new = precision_score(y_test, (new_ensemble >= 0.5).astype(int), zero_division=0)
    ensemble_recall_new = recall_score(y_test, (new_ensemble >= 0.5).astype(int), zero_division=0)
    ensemble_f1_new = f1_score(y_test, (new_ensemble >= 0.5).astype(int), zero_division=0)
        ...
        json.dump(
            {
                ...
                "ensemble_auc_old": roc_auc_score(y_test, old_ensemble),
                "ensemble_auc_new": ensemble_auc_new,
                "ensemble_precision": ensemble_precision_new,
                "ensemble_recall": ensemble_recall_new,
                "ensemble_f1": ensemble_f1_new,
                "brier_old": float(brier_score_loss(y_test, old_ensemble)),
                "brier_new": float(brier_score_loss(y_test, new_ensemble)),
