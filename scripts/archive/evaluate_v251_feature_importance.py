...
def compare_models(df_v250, df_v251):
    ...
    # 合并数据
    df_merge = pd.merge(
        df_v250[['feature', 'importance', 'percentage', 'category']].rename(columns={
            'importance': 'importance_v250',
            'percentage': 'percentage_v250',
            'category': 'category_v250'
        }),
    ...
    df_merge['category'] = df_merge['category_v251'].where(
        df_merge['category_v251'] != 0, 
        df_merge['category_v250']
    )
    ...
    top_changes = df_merge.nlargest(20, 'percentage_change')[['feature', 'percentage_v250', 'percentage_v251', 'percentage_change', 'category']].copy()
    for idx, row in top_changes.iterrows():
        log.info(f"{row['feature']:<30} {row['percentage_v250']:<12.2f}% {row['percentage_v251']:<12.2f}% {row['percentage_change']:<10.2f}% {row['category']:<15}")
