...
# -*- coding: utf-8 -*-
"""
v2.8.1 批量预测脚本（Tushare 对齐版）

与 v2.8.0 预测流程完全一致，仅使用 v2.8.1 模型。
"""
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "prediction" / "v281_stk_factor"


def main():
    parser = argparse.ArgumentParser(description="v2.8.1 批量预测（Tushare 对齐版）")
    ...
    predictor = EnsemblePredictor(model_version="v2.8.1-ensemble")
    ...
    log.success("=" * 80)
    log.success(f"v2.8.1 预测完成: {len(results)} 天")
    log.success("=" * 80)
