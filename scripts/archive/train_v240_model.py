...
def random_split(df, test_ratio=0.2, cal_ratio=0.15):
    ...
    df_train, df_cal = train_test_split(df_train_full, test_size=cal_ratio/(1-test_ratio), random_state=42, stratify=df_train_full['label'])
...
def evaluate(booster, calibrator, X_test, y_test, feature_names):
    ...
    # 检查新增特征的重要性
    new_features = [
        'price_range_pct', 'close_vs_ma10_std', 'days_near_ma10',
        'volume_shrink_ratio', 'ma10_cross_count'
    ]
...
def save_model(booster, calibrator, feature_names, metrics):
    ...
    metadata = {
        ...
        'anti_chasing_config': {
            'pre_t1_return_max': 20,
            'pre_t1_volatility_max': 3
        },
