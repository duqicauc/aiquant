...
def backtest_single_date(pred_date, eval_date, dm):
    ...
    log.info("-" * 65)
    log.info(
        f"{'平均收益':<20} {v231_stats['avg_return']:>+10.2f}%    {v232_stats['avg_return']:>+10.2f}%    {v232_stats['avg_return'] - v231_stats['avg_return']:>+10.2f}%"
    )
    log.info(
        f"{'中位数收益':<20} {v231_stats['median_return']:>+10.2f}%    {v232_stats['median_return']:>+10.2f}%    {v232_stats['median_return'] - v231_stats['median_return']:>+10.2f}%"
    )
    log.info(
        f"{'胜率':<20} {v231_stats['win_rate']:>10.1f}%    {v232_stats['win_rate']:>10.1f}%    {v232_stats['win_rate'] - v231_stats['win_rate']:>+10.1f}%"
    )
    ...
    log.info(
        f"{'追高数量(>9%)':<20} {v231_stats['chase_high_count']:>10}/10    {v232_stats['chase_high_count']:>10}/10    {v232_stats['chase_high_count'] - v231_stats['chase_high_count']:>+10}"
    )
...
def main():
    ...
    if all_results:
        ...
        log.info("-" * 70)
        log.info(
            f"{'平均收益(所有日期平均)':<25} {np.mean(v231_returns):>+10.2f}%    {np.mean(v232_returns):>+10.2f}%    {np.mean(v232_returns) - np.mean(v231_returns):>+10.2f}%"
        )
        log.info(
            f"{'平均胜率(所有日期平均)':<25} {np.mean(v231_win_rates):>10.1f}%    {np.mean(v232_win_rates):>10.1f}%    {np.mean(v232_win_rates) - np.mean(v231_win_rates):>+10.1f}%"
        )
        log.info(
            f"{'平均追高数量':<25} {np.mean(v231_chase):>10.1f}/10    {np.mean(v232_chase):>10.1f}/10    {np.mean(v232_chase) - np.mean(v231_chase):>+10.1f}"
        )
