...
def compare_models(df_v251, df_v252):
    ...
    df_merge = pd.merge(
        ...
        df_v252[["feature", "importance", "percentage", "category"]].rename(
            columns={"importance": "importance_v252", "percentage": "percentage_v252", "category": "category_v252"}
        ),
        on="feature",
        how="outer",
    ).fillna(0)
    ...
    df_merge["category"] = df_merge["category_v252"].where(df_merge["category_v252"] != 0, df_merge["category_v251"])
    ...
    top_changes = df_merge.nlargest(20, "percentage_change")[
        ["feature", "percentage_v251", "percentage_v252", "percentage_change", "category"]
    ].copy()
    for idx, row in top_changes.iterrows():
        log.info(
            f"{row['feature']:<30} {row['percentage_v251']:<12.2f}% {row['percentage_v252']:<12.2f}% {row['percentage_change']:<10.2f}% {row['category']:<15}"
        )
