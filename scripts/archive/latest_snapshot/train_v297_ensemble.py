...
# ============================================================================
MOMENTUM_FEATURES = [
    "mfi", "ema_250", "wr1", "ma_mass", "mass", "ema_5", "trend_slope_34d",
    "momentum_5d", "mtmma", "bias_long", "ma_8d", "ma60", "dmi_pdi", "madpo",
    "dmi_mdi", "dmi_adx", "dmi_adxr", "wr", "mtm", "roc", "maroc", "trix",
    "trma", "expma_12", "expma_50", "xsii_td1", "xsii_td2", "xsii_td3",
    "xsii_td4", "momentum_10d", "momentum_20d", "ema_10", "ema_20", "ema_60",
    "ema_30", "ema_90", "trend_slope_8d", "trend_slope_55d", "ma30", "ma90",
    "ma250", "bias_short", "bias_mid", "ma_34d", "ma_55d", "ma10", "ma5",
    "dfma_dif", "dfma_difma", "macd_dif", "macd_dea", "macd", "rsi_6",
    "rsi_12", "rsi_24", "kdj_k", "kdj_d", "kdj_j",
]

PRICE_VOLUME_FEATURES = [
    "relative_volatility", "vr", "breakout_high_55d", "support_20d",
    "turnover_zscore", "dist_to_resistance_10d", "maemv", "dist_to_support_20d",
    "volume_rsv_20d", "resistance_strength_55d", "volume_price_corr_10d",
    "volume_trend_slope_20d", "volume_price_divergence_strength",
    "volume_price_divergence", "price_down_vol_up_count_10d", "ktn_upper",
    "ktn_down", "taq_up", "taq_down", "boll_upper", "boll_lower",
    "dist_to_resistance_20d", "dist_to_support_10d", "support_strength_10d",
    "resistance_strength_10d", "support_strength_20d", "resistance_strength_20d",
    "breakout_strength_10d", "breakout_strength_20d", "breakout_strength_55d",
    "breakout_volume_strength", "high_volume_breakout", "volume_price_match",
    "vol_10d_avg", "vol_20d_avg", "vol_ratio_10d", "vol_ratio_20d",
    "turnover_f_change", "turnover_zscore_20d", "volume_consistency",
    "relative_volume", "volume_price_sync", "volume_spike_count_10d",
    "obv_trend", "obv_calc", "amount", "turnover_rate", "turnover_rate_f",
    "volume_ratio", "vol", "volatility_8d", "volatility_34d", "volatility_55d",
    "atr", "atr_14", "atr_expansion",
]

MARKET_FEATURES = [
    "market_momentum_10d", "market_pct_chg", "momentum_market_interaction",
    "market_trend", "excess_return_cumsum", "excess_return",
    "excess_return_consistency", "market_volatility_34d", "market_position_20d",
    "market_return_34d", "market_momentum_20d", "market_momentum_5d",
]

POSITION_FEATURES = [
    "price_vs_ma_55d", "price_vs_ma_34d", "price_position_8d",
    "price_position_avg", "price_position_55d", "price_vs_ma_8d",
    "price_position_34d",
]
...
def load_training_data():
    """加载 v297 训练数据（时间序列多行增强版本）"""
    log.info("加载 v297 训练数据...")

    pos_file = PROJECT_ROOT / "data" / "training" / "v297" / "positive_features.csv"
    neg_file = PROJECT_ROOT / "data" / "training" / "v297" / "negative_features.csv"
    hard_file = PROJECT_ROOT / "data" / "training" / "v297" / "hard_negative_features.csv"
    ...
    dead_features = [
        ...
        "market_regime", "breakout_with_volume", "resonance_volume_confirm",
        "days_from_high_20d", "days_from_high_55d", "ma10_cross_count",
    ]
    redundant_features = [
        "obv_calc", "ma5", "bbi", "ktn_mid", "boll_mid",
        "resistance_55d", "support_55d", "resistance_10d", "resistance_20d",
        "price_vs_hist_mean", "return_8d", "return_34d", "return_55d",
        "breakout_strength_10d", "breakout_strength_20d", "max_drawdown_55d",
        "vol_ma20_ratio", "breakout_strength_max", "price_vs_hist_high",
    ]
    ...
    # ========== 5. 合并训练集 ==========
    # v2.9.7: 保留 sample_id 和 days_to_t1（时间序列增强关键列）
    keep_cols = common_cols + ["label", "trade_date", "ts_code"]
    if "sample_id" in df_pos.columns:
        keep_cols.append("sample_id")
    if "days_to_t1" in df_pos.columns:
        keep_cols.append("days_to_t1")

    df = pd.concat(
        [
            df_pos[[c for c in keep_cols if c in df_pos.columns]],
            df_neg[[c for c in keep_cols if c in df_neg.columns]],
            df_hard[[c for c in keep_cols if c in df_hard.columns]],
        ],
        ignore_index=True,
    )
...
def get_feature_columns(df, feature_cols):
    ...
    exclude_cols = [
        ...
        "weekly_volume_2",
        "weekly_volume_3",
        # v2.9.7: days_to_t1 作为时间序列增强的关键特征保留
        # "days_to_t1",
        "breakout_high_10d",
        "breakout_high_20d",
        "breakout_ma10",
...
def time_series_cv_splits(df, n_splits=5, train_days=365, val_days=30, test_days=30):
    """
    生成时间序列交叉验证的划分 —— v2.9.7 适配多行数据
    核心：按 sample_id 的 T1 日期统一划分，确保同一样本的所有行进入同一 fold
    ...
    # 计算每个 sample_id 的 T1 日期（最大 trade_date）
    sample_t1_date = df.groupby("sample_id")["trade_date"].max().reset_index()
    sample_t1_date.columns = ["sample_id", "t1_date"]
    sample_t1_date = sample_t1_date.sort_values("t1_date")

    unique_dates = sorted(sample_t1_date["t1_date"].dt.date.unique())
    ...
    for i in range(n_splits):
        ...
        # 根据 T1 日期决定 sample_id 属于哪个 fold
        train_sample_ids = set(sample_t1_date[sample_t1_date["t1_date"].dt.date.isin(train_dates)]["sample_id"])
        val_sample_ids = set(sample_t1_date[sample_t1_date["t1_date"].dt.date.isin(val_dates)]["sample_id"])
        test_sample_ids = set(sample_t1_date[sample_t1_date["t1_date"].dt.date.isin(test_dates)]["sample_id"])

        # 将划分结果应用到所有行（同一 sample_id 的所有行进入同一 fold）
        train = df[df["sample_id"].isin(train_sample_ids)].copy()
        val = df[df["sample_id"].isin(val_sample_ids)].copy()
        test = df[df["sample_id"].isin(test_sample_ids)].copy()

        if len(train) > 0 and len(val) > 0 and len(test) > 0:
            splits.append((train, val, test))
            log.info(
                f"Fold {len(splits)}: 训练 {len(train_dates)}天/{len(train)}行/{len(train_sample_ids)}样本, "
                f"验证 {len(val_dates)}天/{len(val)}行/{len(val_sample_ids)}样本, "
                f"测试 {len(test_dates)}天/{len(test)}行/{len(test_sample_ids)}样本"
            )
...
def train_xgboost(X_train, y_train, X_val, y_val, feature_names, sample_weight=None):
    ...
    log.info(f"  XGBoost AUC={auc:.4f}, Brier={brier:.4f}, best_iteration={model.best_iteration}, features={len(feature_names)}")
...
def train_lightgbm(X_train, y_train, X_val, y_val, feature_names, sample_weight=None):
    ...
    auc = roc_auc_score(y_val, y_pred)
    brier = brier_score_loss(y_val, y_pred)
    log.info(f"  LightGBM AUC={auc:.4f}, Brier={brier:.4f}, best_iteration={model.best_iteration}, features={len(feature_names)}")
...
def train_catboost(X_train, y_train, X_val, y_val, feature_names, sample_weight=None):
    ...
    auc = roc_auc_score(y_val, y_pred)
    brier = brier_score_loss(y_val, y_pred)
    log.info(f"  CatBoost AUC={auc:.4f}, Brier={brier:.4f}, best_iteration={model.get_best_iteration()}, features={len(feature_names)}")
...
# 训练后验证 Gates（放宽）
# ============================================================================
def validate_ensemble_gates(models_dict, weights, X_test, y_test, feature_names_dict, feature_indices_dict, max_disagreement=MAX_DISAGREEMENT):
    """训练后验证 Gates"""
    log.info("\n" + "=" * 80)
...
def main():
    log.info("=" * 80)
    log.info("v2.9.7 集成模型训练 —— 时间序列多行增强")
    log.info("=" * 80)
    ...
    xgb_model, xgb_auc, xgb_brier = train_xgboost(
        ...
        valid_features, sample_weight=sample_weights
    )
    ...
    passed, test_preds, ensemble = validate_ensemble_gates(
        models_dict, weights, X_test_full, y_test,
        {"xgboost": valid_features, "lightgbm": valid_features, "catboost": valid_features},
        {"xgboost": list(range(len(valid_features))), "lightgbm": list(range(len(valid_features))), "catboost": list(range(len(valid_features)))}
    )
