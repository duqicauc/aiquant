...
def main():
    ...
    log.info(
        f"校准概率范围: [{df_top10['calibrated_probability'].min():.4f}, {df_top10['calibrated_probability'].max():.4f}]"
    )
    log.info(
        f"预期收益评分范围: [{df_top10['expected_return_score'].min():.4f}, {df_top10['expected_return_score'].max():.4f}]"
    )
