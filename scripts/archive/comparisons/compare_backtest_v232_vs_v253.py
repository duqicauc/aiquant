...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="输出对比报告路径（默认 results/backtest_v232_vs_v253_{start}_{end}.md）",
    )
    ...
    if metrics_v232_d is None:
        log.warning(
            "v232 每日回测数据不存在，请先运行: python scripts/backtest_v232_only.py --start-date %s --end-date %s",
            start_date,
            end_date,
        )
    if metrics_v253_d is None:
        log.warning(
            "v253 每日回测数据不存在。请先: 1) python scripts/run_predict_v253_batch.py --start-date %s --end-date %s  2) python scripts/backtest_v253_only.py --start-date %s --end-date %s",
            start_date,
            end_date,
            start_date,
            end_date,
    ...
    if metrics_v232_d and metrics_v253_d:
        data.append(
            ("最终资产(元)", f"{metrics_v232_d['final_assets']:,.0f}", f"{metrics_v253_d['final_assets']:,.0f}")
        )
        data.append(
            ("收益率(%)", f"{metrics_v232_d['final_return_pct']:+.2f}", f"{metrics_v253_d['final_return_pct']:+.2f}")
        )
        data.append(
            ("最大回撤(%)", f"{metrics_v232_d['max_drawdown_pct']:.2f}", f"{metrics_v253_d['max_drawdown_pct']:.2f}")
        )
        data.append(
            (
                "最大回撤日期",
                str(metrics_v232_d.get("max_drawdown_date") or "-"),
                str(metrics_v253_d.get("max_drawdown_date") or "-"),
            )
        )
    if metrics_v232_o and metrics_v253_o:
        data.append(
            ("买入次数", str(metrics_v232_o.get("total_buys", "-")), str(metrics_v253_o.get("total_buys", "-")))
        )
        data.append(
            ("卖出次数", str(metrics_v232_o.get("total_sells", "-")), str(metrics_v253_o.get("total_sells", "-")))
        )
        data.append(
            ("胜率(%)", f"{metrics_v232_o.get('win_rate_pct', 0):.2f}", f"{metrics_v253_o.get('win_rate_pct', 0):.2f}")
        )
        data.append(
            (
                "平均每笔盈亏(元)",
                f"{metrics_v232_o.get('avg_profit', 0):+,.0f}",
                f"{metrics_v253_o.get('avg_profit', 0):+,.0f}",
            )
        )
        data.append(
            (
                "平均每笔盈亏(%)",
                f"{metrics_v232_o.get('avg_profit_pct', 0):+.2f}",
                f"{metrics_v253_o.get('avg_profit_pct', 0):+.2f}",
            )
        )
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "**规则**: 前一日选股 Top10 当日开盘买，跌出 Top50 且连续两日收盘<MA5 在 T2 收盘卖；30万/只，先买后卖。\n\n"
        )
