...
基于v2.5.0优化版，使用v2.5.2模型（修复days_to_t1数据泄露问题）：
...
def load_model():
    """加载v2.5.2模型"""
    model_dir = PROJECT_ROOT / 'data' / 'models' / 'breakout_launch_scorer' / 'versions' / 'v2.5.2' / 'model'
...
def main():
    parser = argparse.ArgumentParser(description='v2.5.2模型预测')
    ...
    log.info("="*80)
    log.info(f"v2.5.2模型预测 - {predict_date}")
    log.info("="*80)
    ...
    log.success(f"✓ 模型加载成功: {len(feature_names)} 特征（已修复days_to_t1数据泄露）")
    ...
    output_file = output_dir / f'v2.5.2_top10_{predict_date}.csv'
    df_top10.to_csv(output_file, index=False, encoding='utf-8-sig')
    log.success(f"\n💾 Top10结果已保存: {output_file}")
    
    # 保存完整结果
    full_output_file = output_dir / f'v2.5.2_full_{predict_date}.csv'
