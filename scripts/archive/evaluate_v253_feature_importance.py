...
def compare_models(df_v252, df_v253):
    ...
    df_merge = pd.merge(
        ...
        df_v253[["feature", "importance", "percentage", "category"]].rename(
            columns={"importance": "importance_v253", "percentage": "percentage_v253", "category": "category_v253"}
        ),
        on="feature",
        how="outer",
    ).fillna(0)
    ...
    df_merge["category"] = df_merge["category_v253"].where(df_merge["category_v253"] != 0, df_merge["category_v252"])
    ...
    top_changes = df_merge.nlargest(20, "percentage_change")[
        ["feature", "percentage_v252", "percentage_v253", "percentage_change", "category"]
    ].copy()
    for idx, row in top_changes.iterrows():
        log.info(
            f"{row['feature']:<30} {row['percentage_v252']:<12.2f}% {row['percentage_v253']:<12.2f}% {row['percentage_change']:<10.2f}% {row['category']:<15}"
        )
...
def main():
    ...
    log.info("-" * 60)
    log.info(
        f"{'AUC':<20} {metadata_v252['metrics']['auc']:<15.4f} {metadata_v253['metrics']['auc']:<15.4f} {metadata_v253['metrics']['auc'] - metadata_v252['metrics']['auc']:<+10.4f}"
    )
    log.info(
        f"{'Precision':<20} {metadata_v252['metrics']['precision']:<15.4f} {metadata_v253['metrics']['precision']:<15.4f} {metadata_v253['metrics']['precision'] - metadata_v252['metrics']['precision']:<+10.4f}"
    )
    log.info(
        f"{'Recall':<20} {metadata_v252['metrics']['recall']:<15.4f} {metadata_v253['metrics']['recall']:<15.4f} {metadata_v253['metrics']['recall'] - metadata_v252['metrics']['recall']:<+10.4f}"
    )
    log.info(
        f"{'F1':<20} {metadata_v252['metrics']['f1']:<15.4f} {metadata_v253['metrics']['f1']:<15.4f} {metadata_v253['metrics']['f1'] - metadata_v252['metrics']['f1']:<+10.4f}"
    )
