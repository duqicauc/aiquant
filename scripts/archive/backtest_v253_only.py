...
策略逻辑与 v232 回测一致，仅选股数据源改为 v253（v2.5.3_full）单模型结果：
1. 选股与买入日对应：当日买入使用「前一交易日」的v253选股结果（如29号买入用28号选股结果），避免未来数据。
2. 第1天：若前一交易日v253结果不存在，则当日跳过，实际首日建仓为次日。
...
def load_v253_predictions(date: str, top_n: int = 50) -> Optional[pd.DataFrame]:
    """
    加载 v253 单模型预测结果（v2.5.3_full_{date}.csv）。
    ...
    file_path = results_dir / f'v2.5.3_full_{date}.csv'

    if not file_path.exists():
        log.warning(f"v253选股结果不存在: {file_path}")
        return None

    try:
        df = pd.read_csv(file_path, encoding='utf-8-sig')
        if 'final_score' not in df.columns:
            log.error("v253结果缺少 final_score 列")
            return None
        df = df.sort_values('final_score', ascending=False).head(top_n)
        return df
    except Exception as e:
        log.error(f"读取v253选股结果失败: {e}")
        return None
...
def get_stock_price(date: str, ts_code: str, dm: DataManager,
    ...
    """获取股票在指定日期的收盘价。优先从预测结果中获取，否则从 DataManager 获取。"""
...
def backtest_v253_only_strategy(
    start_date: str,
    end_date: str,
    ...
    """
    v253 单模型选股回测（与 v232 回测规则一致，仅数据源为 v2.5.3_full）
    """
    log.info("=" * 80)
    log.info("v253单模型选股回测")
    log.info("=" * 80)
    ...
    for i, date in enumerate(trading_dates):
        ...
        df_pred = load_v253_predictions(signal_date, top_n=top_n_hold)
        if df_pred is None or df_pred.empty:
            log.warning(f"选股日{signal_date}的v253结果不存在或为空，当日{date}无法买入/调仓，跳过")
            if i > 0 and trading_dates[i - 1] in predictions_cache:
                df_pred = predictions_cache[trading_dates[i - 1]]
            else:
                continue
        else:
            log.info(f"选股日: {signal_date} -> 买入日: {date} (使用 v2.5.3_full_{signal_date}.csv)")
        ...
        for ts_code in stocks_to_buy:
            ...
            buy_reason = f"选股日{signal_date}的v253 Top10，当日开盘价买入"
        ...
        buy_reason_remark = f"选股日{signal_date}的v253 Top10新进{buy_count_today}只" if buy_count_today else "无"
...
def generate_report(result: Dict, output_dir: Path):
    """生成 v253 单模型回测报告（输出文件名带 v253_only 前缀）"""
    log.info("\n" + "=" * 80)
    log.info("v253单模型回测结果汇总")
    log.info("=" * 80)
    ...
    daily_file = output_dir / f"backtest_v253_only_daily_{result['start_date']}_{result['end_date']}.csv"
    result['daily_records'].to_csv(daily_file, index=False, encoding='utf-8-sig')
    log.success(f"\n✓ 每日记录已保存: {daily_file}")

    if not result['operations_log'].empty:
        operations_file = output_dir / f"backtest_v253_only_operations_{result['start_date']}_{result['end_date']}.csv"
        result['operations_log'].to_csv(operations_file, index=False, encoding='utf-8-sig')
        log.success(f"✓ 操作日志已保存: {operations_file}")

    report_file = output_dir / f"backtest_v253_only_report_{result['start_date']}_{result['end_date']}.md"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write(f"# v253单模型选股回测报告\n\n")
        f.write(f"## 回测区间\n\n")
        f.write(f"- 开始日期: {result['start_date']}\n")
        f.write(f"- 结束日期: {result['end_date']}\n\n")
        f.write(f"## 策略参数\n\n")
        f.write(f"- 选股数据: v2.5.3_full（v253单模型）\n")
...
def main():
    parser = argparse.ArgumentParser(description='v253单模型选股回测（与v232同规则，用于对比）')
    ...
    result = backtest_v253_only_strategy(
