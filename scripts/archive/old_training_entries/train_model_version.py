...
"""
        ...
        log.success("  ✓ 备份说明: BACKUP_README.md")
    ...
    def _generate_version_report(self, use_advanced_factors: bool, neg_version: str):
        ...
        report = {
            ...
            "backup": {"location": str(self.training_data_dir), "timestamp": datetime.now().isoformat()},
        }
    ...
    def _print_summary(self):
        ...
        log.info("   3. 运行预测: python scripts/predict_v240.py --date YYYYMMDD")
        log.info("")
...
使用示例:
  ...
  python scripts/train_model_version.py --version v1.5.0 --no-advanced-factors
    ...
    parser.add_argument("--version", type=str, required=True, help="模型版本号（如 v1.5.0）")
    parser.add_argument(
        "--model-name", type=str, default="breakout_launch_scorer", help="模型名称（默认: breakout_launch_scorer）"
    )
    parser.add_argument("--neg-version", type=str, default="v2", choices=["v1", "v2"], help="负样本版本（默认: v2）")
    parser.add_argument("--skip-data-prep", action="store_true", help="跳过数据准备步骤（使用现有数据）")
    parser.add_argument("--skip-training", action="store_true", help="跳过训练步骤")
    parser.add_argument("--skip-validation", action="store_true", help="跳过 walk-forward 验证步骤")
    parser.add_argument("--backup-only", action="store_true", help="只执行备份（用于已有版本）")
    parser.add_argument("--no-advanced-factors", action="store_true", help="不使用高级技术因子")

    args = parser.parse_args()

    # 创建训练器并执行
    trainer = ModelVersionTrainer(version=args.version, model_name=args.model_name)
