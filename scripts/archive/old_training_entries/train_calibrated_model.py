...
def train_base_model(X_train, y_train, X_val, y_val):
    ...
    booster = xgb.train(params, dtrain, num_boost_round=300, evals=watchlist, early_stopping_rounds=30, verbose_eval=50)
...
def evaluate_model(booster, calibrator, X_test, y_test, feature_names):
    ...
    for thresh in [0.9, 0.8, 0.7, 0.6, 0.5]:
        ...
        log.info(
            f"  概率>={thresh}: 原始{raw_high.sum()}个(真实{raw_acc_high:.1%}) vs 校准后{cal_high.sum()}个(真实{cal_acc_high:.1%})"
        )
