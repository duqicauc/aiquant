...
def main():
    ...
    for i, (_, row) in enumerate(top100.iterrows(), 1):
        log.info(
            f"{i:3d}  | {row['ts_code']:<10} | {row['name']:<8} | {row['calibrated_probability']:.2%} | {row['risk_score']:.2f} | {row['final_score']:.4f} | {row['close_price']:.2f} | {row['return_34d']:+.1f}%"
        )
    ...
    log.info("=" * 80)
    log.info(
        f"校准概率 - 最高: {top100['calibrated_probability'].max():.2%}, 最低: {top100['calibrated_probability'].min():.2%}, 平均: {top100['calibrated_probability'].mean():.2%}"
    )
    log.info(
        f"风险系数 - 最高: {top100['risk_score'].max():.2f}, 最低: {top100['risk_score'].min():.2f}, 平均: {top100['risk_score'].mean():.2f}"
    )
    log.info(
        f"34日涨幅 - 最高: {top100['return_34d'].max():.1f}%, 最低: {top100['return_34d'].min():.1f}%, 平均: {top100['return_34d'].mean():.1f}%"
    )
