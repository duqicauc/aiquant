...
def process_single_stock(dm, ts_code, name, predict_date, feature_names, booster, calibrator):
    ...
    try:
        ...
        # v2.3.1: 双目标排序（校准概率 + 预期收益）
        # 归一化预期收益评分到0-1范围（假设已经在0-1之间）
        expected_return_norm = float(np.clip(expected_return_score, 0, 1))
        
        # 双目标综合评分：0.5 * 校准概率 + 0.5 * 预期收益
...
def main():
    parser = argparse.ArgumentParser(description='v2.3.1模型预测 - Top10优化版')
    parser.add_argument('--date', type=str, default='20251212', help='预测日期 (YYYYMMDD)')
    args = parser.parse_args()
    
    predict_date = args.date
    
    log.info("="*80)
    log.info(f"v2.3.1模型预测 - Top10优化版 - {predict_date}")
    log.info("="*80)
    ...
    # 加载模型
    log.info("\n📦 加载v2.3.0模型（v2.3.1使用v2.3.0的模型和校准器）...")
    ...
    # 显示Top10
    log.info("\n" + "="*80)
    log.info("🏆 v2.3.1 Top10 推荐（双目标排序：校准概率 + 预期收益）")
    log.info("="*80)
    log.info(f"\n{'排名':<4} {'代码':<12} {'名称':<10} {'综合评分':<10} {'校准概率':<10} {'预期收益':<10} {'34日涨幅':<10}")
    log.info("-" * 80)
    ...
    for i, (_, row) in enumerate(df_top10.iterrows(), 1):
        log.info(
            ...
            f"{row['expected_return_score']:<10.4f} {row['return_34d']:>+9.1f}%"
        )
    ...
    output_file = output_dir / f'v2.3.1_top10_{predict_date}.csv'
    df_top10.to_csv(output_file, index=False, encoding='utf-8-sig')
    log.success(f"\n💾 Top10结果已保存: {output_file}")
    
    # 保存完整结果（用于后续评估）
    full_output_file = output_dir / f'v2.3.1_full_{predict_date}.csv'
    ...
    log.info("📊 Top10 统计信息")
    log.info("="*80)
    log.info(f"综合评分范围: [{df_top10['final_score'].min():.4f}, {df_top10['final_score'].max():.4f}]")
    log.info(f"校准概率范围: [{df_top10['calibrated_probability'].min():.4f}, {df_top10['calibrated_probability'].max():.4f}]")
    log.info(f"预期收益评分范围: [{df_top10['expected_return_score'].min():.4f}, {df_top10['expected_return_score'].max():.4f}]")
    log.info(f"34日涨幅范围: [{df_top10['return_34d'].min():.1f}%, {df_top10['return_34d'].max():.1f}%]")
