...
用法：
    python scripts/screen_existing_predictions.py --file data/prediction/results/v270_ensemble_top50_20260116.csv --date 20260116
"""
import sys
import argparse
...
def screen_predictions(prediction_file: str, trade_date: str):
    """
    对已有预测结果进行基本面筛选
    ...
    if len(failed) > 0:
        log.info("\n" + "="*80)
        log.info(f"未通过基本面筛选的股票 ({len(failed)}只)")
        log.info("="*80)
        
        log.info(f"\n{'代码':<12} {'名称':<10} {'未通过原因':<50}")
        log.info("-" * 80)
        
        for _, row in failed.iterrows():
            reason = row.get('fundamental_reason', '未知原因')
            name = row.get('name', '')
            log.info(f"{row['ts_code']:<12} {name:<10} {reason:<50}")
    ...
    # 保存通过筛选的股票
    if len(passed) > 0:
        output_file = output_dir / f"{Path(prediction_file).stem}_fundamental_screened_{trade_date}.csv"
        passed.to_csv(output_file, index=False, encoding='utf-8-sig')
        log.success(f"\n✓ 通过筛选的股票已保存: {output_file}")
    
    # 保存完整筛选结果（包含通过/未通过信息）
    full_output_file = output_dir / f"{Path(prediction_file).stem}_fundamental_full_{trade_date}.csv"
...
def main():
    parser = argparse.ArgumentParser(description='对已有预测结果进行基本面筛选')
    ...
    screen_predictions(args.file, args.date)
