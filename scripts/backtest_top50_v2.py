...
def compute_random_benchmark(
    df_preds, all_stocks, price_map, all_dates, date_to_idx, holding_days, cost=0.0, n_trials=500
):
    ...
    for _ in range(n_trials):
        ...
        for d in dates:
            # 当天有价格数据的股票
            available = [s for s in all_stocks if s in price_map and d in price_map[s]]
...
def calc_metrics(arr):
    ...
    return {
        ...
        "profit_loss_ratio": (
            (np.mean(wins) / abs(np.mean(losses))) if len(losses) > 0 and abs(np.mean(losses)) > 0 else float("inf")
        ),
...
def main():
    ...
    print("=" * 90)
    print("  AIQuant Top50 — 含交易成本回测 + 随机基准")
    comm = COMMISSION_RATE * 100
    stamp = STAMP_TAX_RATE * 100
    total_cost = TOTAL_COST_ROUND_TRIP * 100
    print(f"  交易成本: 佣金{comm:.2f}%×2 + 印花税{stamp:.1f}% = {total_cost:.1f}%/次")
    print(f"  区间: {preds['date'].min()} ~ {preds['date'].max()}")
    print(f"  总预测: {len(preds)}")
    print("  随机基准: 每次500次模拟, 每日50只")
    print("=" * 90)
    ...
    for hd, hn in [(1, "1d"), (2, "2d"), (3, "3d"), (4, "4d"), (5, "5d"), (10, "10d"), (15, "15d"), (20, "20d")]:
        # Top50
        ...
        # 随机基准
        random_ret = compute_random_benchmark(
            preds, all_stocks, price_map, all_dates, date_to_idx, hd, TOTAL_COST_ROUND_TRIP, n_trials=500
        )
        ...
        print(
            f"  {hn:>6}  | {m.get('avg_ret','N/A'):>+8.2f}% | {m.get('win_rate','N/A'):>6.1f}% | "
    ...
    for hn, hd in [("1d", 1), ("3d", 3)]:
        ...
        print(
            f"  {'波动率(年化)':<20} {m_t['std'] * (252**0.5):<8.1f}%{'':>15} {m_r['std'] * (252**0.5):<8.1f}%{'':>15}"
        )
        ...
        if random_rets_by_day:
            ...
            cum = (1 + np.array(random_rets_by_day) / 100).prod() - 1
            print(f"    随机组合: {rp_avg:+.3f}% | 夏普: {rp_sharpe:.2f} | 累计: {cum*100:+.1f}%")
    ...
    print("\n  ▶ 与随机基准对比:")
    bench_3d = compute_random_benchmark(
        preds, all_stocks, price_map, all_dates, date_to_idx, 3, TOTAL_COST_ROUND_TRIP, n_trials=500
    )
    ...
    if sharpe_val > 0.5 and avg_return > 0.2:
        print("    ✅ 可商业化做「短线信号工具」定位，但需搭配组合风控")
    elif sharpe_val > 0.2 and avg_return > 0:
        print("    🟡 信号质量勉强合格，但产品定位需要非常精准" "\n      建议定位：「概率参考信号」，不保证单次收益")
    else:
        print("    ❌ 数据不足以支撑商业化，建议先改进模型")
