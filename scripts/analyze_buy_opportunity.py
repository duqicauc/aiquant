...
def analyze_buy_opportunity(ts_code, name, predict_date, close, pct_chg, rsi_6, consecutive_limit_up=0, hot_sector=""):
    """
    分析买入机会
    ...
    if consecutive_limit_up >= 3:
        ...
        risk_score += 8
        risk_factors.append("⚠️ 昨日涨停，需关注今日表现")
    else:
        positive_factors.append("✓ 无连续涨停，风险可控")
    ...
    if hot_sector:
        ...
        if pct_chg >= 5:
            risk_score += 5
            risk_factors.append("⚠️ 热门板块+高涨幅，需警惕资金获利了结")
