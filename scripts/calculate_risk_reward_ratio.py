...
for h in holdings:
    ...
    print(f"  ---")
    print(f"  止盈目标1: {h['target1']:.2f}元 ({(h['target1']-current)/current*100:+.2f}%) → 止盈{h['target1_ratio']*100:.0f}%仓位")
    print(f"  止盈目标2: {h['target2']:.2f}元 ({(h['target2']-current)/current*100:+.2f}%) → 止盈{h['target2_ratio']*100:.0f}%仓位")
    if 'target3' in h:
        print(f"  止盈目标3: {h['target3']:.2f}元 ({(h['target3']-current)/current*100:+.2f}%) → 止盈{h['target3_ratio']*100:.0f}%仓位")
...
print("\n1. 盈亏比优秀的股票（>1.5:1）:")
excellent_stocks = [h for h in holdings if (h['target1']-h['current'])/h['current']*100 / abs((h['stop_loss']-h['current'])/h['current']*100) >= 1.5]
if excellent_stocks:
...
print("\n2. 盈亏比较差的股票（<1:1）:")
poor_stocks = [h for h in holdings if (h['target1']-h['current'])/h['current']*100 / abs((h['stop_loss']-h['current'])/h['current']*100) < 1]
if poor_stocks:
    for h in poor_stocks:
        risk_reward_val = (h['target1']-h['current'])/h['current']*100 / abs((h['stop_loss']-h['current'])/h['current']*100)
        print(f"   ⚠️ {h['name']} (盈亏比{risk_reward_val:.2f}:1)")
        if h['current_profit_pct'] > 5:
            print(f"      建议: 已盈利{h['current_profit_pct']:.2f}%，考虑提前止盈部分仓位")
        else:
            print(f"      建议: 适当提高止盈目标或收紧止损")
else:
    print("   无")
