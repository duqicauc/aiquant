...
def record_buy(ts_code: str, date: str, price: float, quantity: int, amount: float, 
               batch: int = 1, note: str = ""):
    """记录买入"""
...
def record_sell(ts_code: str, date: str, price: float, quantity: int, amount: float, 
                note: str = ""):
    """记录卖出"""
    record_file = PROJECT_ROOT / 'data' / 'prediction' / 'trading_plans' / f'{ts_code}_trades.json'
...
def show_status(ts_code: str, current_price: Optional[float] = None):
    ...
    if total_sell_amount > 0:
        realized_profit = total_sell_amount - (total_buy_amount * total_sell_quantity / total_buy_quantity if total_buy_quantity > 0 else 0)
    ...
    if records['buys']:
        ...
        for b in records['buys']:
            log.info(f"{b['date']:<12} {b['price']:<10.2f} {b['quantity']:<10} {b['amount']:<12.2f} {b.get('batch', 1):<6} {b.get('note', '')}")
    ...
    if records['sells']:
        ...
        for s in records['sells']:
            log.info(f"{s['date']:<12} {s['price']:<10.2f} {s['quantity']:<10} {s['amount']:<12.2f} {s.get('note', '')}")
...
def main():
    ...
    if args.action == 'buy':
        ...
        record_buy(args.ts_code, args.date, args.price, args.quantity, amount, 
