    ...
    def calc_sample_features(group):
        ...
        if 'close' in g.columns:
            ...
            g['price_position_8d'] = np.where(
                g['high_8d'] > g['low_8d'],
                (g['close'] - g['low_8d']) / (g['high_8d'] - g['low_8d']),
                0.5
            )
            g['price_position_34d'] = np.where(
                g['high_34d'] > g['low_34d'],
                (g['close'] - g['low_34d']) / (g['high_34d'] - g['low_34d']),
                0.5
            )
            g['price_position_55d'] = np.where(
                g['high_55d'] > g['low_55d'],
                (g['close'] - g['low_55d']) / (g['high_55d'] - g['low_55d']),
                0.5
            )
            ...
            g['price_vs_ma_8d'] = (g['close'] - g['close'].rolling(8, min_periods=3).mean()) / g['close'].rolling(8, min_periods=3).mean() * 100
        ...
        g['support_strength_10d'] = np.where(
            g['support_10d'] > 0,
            (g['close'] - g['support_10d']) / g['support_10d'] * 100,
            0
        )
        g['resistance_strength_10d'] = np.where(
            g['resistance_10d'] > 0,
            (g['resistance_10d'] - g['close']) / g['resistance_10d'] * 100,
            0
        )
        g['support_strength_20d'] = np.where(
            g['support_20d'] > 0,
            (g['close'] - g['support_20d']) / g['support_20d'] * 100,
            0
        )
        g['resistance_strength_20d'] = np.where(
            g['resistance_20d'] > 0,
            (g['resistance_20d'] - g['close']) / g['resistance_20d'] * 100,
            0
        )
...
def calculate_breakout_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'close' in df.columns and 'high_10d' in df.columns:
        df['breakout_strength_10d'] = np.where(
            df['high_10d'] > 0,
            (df['close'] - df['high_10d']) / df['high_10d'] * 100,
            0
        )
    else:
    ...
    if 'close' in df.columns and 'high_20d' in df.columns:
        df['breakout_strength_20d'] = np.where(
            df['high_20d'] > 0,
            (df['close'] - df['high_20d']) / df['high_20d'] * 100,
            0
        )
    else:
    ...
    if 'close' in df.columns and 'high_55d' in df.columns:
        df['breakout_strength_55d'] = np.where(
            df['high_55d'] > 0,
            (df['close'] - df['high_55d']) / df['high_55d'] * 100,
            0
        )
    else:
...
def get_market_data(dm: DataManager, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    try:
        ...
        df_market['market_trend'] = np.where(df_market['ma5'] > df_market['ma20'], 1, -1) if 'ma5' in df_market.columns and 'ma20' in df_market.columns else 0
        ...
        df_market['market_regime'] = np.where(
            df_market['market_momentum_20d'] > 5, 1,
            np.where(df_market['market_momentum_20d'] < -5, -1, 0)
        )
        ...
        df_market['market_position_20d'] = np.where(
            roll_max > roll_min,
            (df_market['close'] - roll_min) / (roll_max - roll_min),
            0.5
        )
        
        return df_market[['trade_date', 'market_pct_chg', 'market_return_34d', 
