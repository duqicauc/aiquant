...
def recommend_stocks_from_combined(combined_file, date=None, prefer_hot_sector=True, 
                                   prefer_high_return=True, top_n=2):
    """
    从互补策略结果中选择股票
    ...
    df['macd_signal'] = df['ts_code'].apply(
        lambda x: tech_indicators_dict.get(x, {}).get('macd_signal', 'neutral')
    )
    df['kdj_signal'] = df['ts_code'].apply(
        lambda x: tech_indicators_dict.get(x, {}).get('kdj_signal', 'neutral')
    )
    df['macd_dif'] = df['ts_code'].apply(
        lambda x: tech_indicators_dict.get(x, {}).get('macd_dif', 0)
    )
    df['macd_dea'] = df['ts_code'].apply(
        lambda x: tech_indicators_dict.get(x, {}).get('macd_dea', 0)
    )
    df['kdj_k'] = df['ts_code'].apply(
        lambda x: tech_indicators_dict.get(x, {}).get('kdj_k', 50)
    )
    df['kdj_d'] = df['ts_code'].apply(
        lambda x: tech_indicators_dict.get(x, {}).get('kdj_d', 50)
    )
    ...
    for idx, (_, row) in enumerate(top_stocks.iterrows(), 1):
        ...
        # 双重信号提示
        if macd_signal == 'golden_cross' and kdj_signal == 'golden_cross':
            log.info(f"  🎯 双重金叉！MACD和KDJ同时金叉，强烈买入信号")
        elif macd_signal == 'death_cross' and kdj_signal == 'death_cross':
            log.info(f"  ⚠️ 双重死叉！MACD和KDJ同时死叉，强烈卖出信号")
    ...
    # 选择要保存的列
    output_cols = ['ts_code', 'name', 'close', 'source', 'risk_level', 
                   'dual_score', 'selection_score', 'v270_prob', 
                   'hot_sectors', 'rsi_6', 'pct_chg',
                   'macd_signal', 'kdj_signal', 'macd_dif', 'macd_dea', 'kdj_k', 'kdj_d']
...
def main():
    ...
    parser.add_argument('--file', type=str, 
                       help='互补策略结果文件路径（如果不提供，会自动查找最新文件）')
    parser.add_argument('--date', type=str, 
                       help='日期(YYYYMMDD)，用于查找文件或输出文件名')
    parser.add_argument('--top-n', type=int, default=2,
                       help='推荐股票数量（默认2）')
    parser.add_argument('--prefer-hot', action='store_true', default=True,
                       help='偏好热门板块（默认开启）')
    parser.add_argument('--no-prefer-hot', dest='prefer_hot', action='store_false',
                       help='不偏好热门板块')
    parser.add_argument('--prefer-return', action='store_true', default=True,
                       help='偏好高收益（默认开启）')
    parser.add_argument('--no-prefer-return', dest='prefer_return', action='store_false',
                       help='不偏好高收益（偏好稳健）')
    ...
    recommend_stocks_from_combined(
        combined_file, 
        date, 
        prefer_hot_sector=args.prefer_hot,
        prefer_high_return=args.prefer_return,
        top_n=args.top_n
    )
