...
# -*- coding: utf-8 -*-
"""
v2.9.2 CatBoost 单模型 — 使用 v291 超参数验证

核心假设：v292 与 v291 的 AUC 差距（0.9524 vs 0.9776）可能来自超参数差异：
  - v291: lr=0.1, l2=3.0, auto_class_weights="Balanced"
  - v292: lr=0.05, l2=5.0, 无 auto_class_weights

本脚本在 v292 数据上应用 v291 的超参数，验证差距是否可解释。

Usage:
    python scripts/train_v292_catboost_v291_params.py
"""
...
VERSION = "v2.9.2-catboost-v291-params"
...
def train_catboost_v291_params(X_train, y_train, X_val, y_val):
    """训练 CatBoost — 使用 v291 超参数"""
    log.info("\n训练 CatBoost（v291 超参数）...")
    log.info("  关键差异 vs v292-conservative:")
    log.info("    learning_rate: 0.05 → 0.1")
    log.info("    l2_leaf_reg:   5.0 → 3.0")
    log.info("    新增: auto_class_weights='Balanced'")
    log.info("    移除: scale_pos_weight")

    model = CatBoostClassifier(
        iterations=500,
        depth=6,
        learning_rate=0.10,  # ← v291: 更快学习
        l2_leaf_reg=3.0,  # ← v291: 更弱正则
        border_count=128,
        auto_class_weights="Balanced",  # ← v291 关键: 自动类别权重
        random_seed=42,
        verbose=False,
...
def save_model(model, feature_names, calibrator, metrics, hard_ratio):
    ...
    metadata = {
        ...
        "description": "v2.9.2-catboost-v291-params - 使用v291超参(lr=0.1,l2=3.0,auto_class_weights=Balanced)",
        "hyperparameters": {
            "iterations": 500,
            "depth": 6,
            "learning_rate": 0.1,
...
def main():
    log.info("=" * 80)
    log.info(f"训练 {VERSION}（CatBoost + v291 超参数）")
    log.info("=" * 80)
    ...
    log.info("\n" + "=" * 80)
    log.info("训练 CatBoost（v291 超参数）")
    log.info("=" * 80)
    model, val_auc = train_catboost_v291_params(X_train, y_train, X_cal, y_cal)
    ...
    comparisons = {
        "v2.7.1-conservative": {"auc": 0.9554, "f1": 0.8079},
        "v2.9.1-catboost(sub)": {"auc": 0.9776, "f1": 0.8626},
        "v2.9.2-conservative": {"auc": 0.9524, "f1": 0.7946},
    }

    log.info(f"\n{'版本':<25} {'AUC':>8} {'F1':>8}")
    log.info("-" * 45)
    for name, m in comparisons.items():
        log.info(f"{name:<25} {m['auc']:>8.4f} {m['f1']:>8.4f}")
    log.info(f"{'v2.9.2-v291-params(当前)':<25} {metrics_cal['auc']:>8.4f} {metrics_cal['f1']:>8.4f}")

    # 判断
    log.info("\n" + "=" * 80)
    log.info("结论判断")
    log.info("=" * 80)
    if metrics_cal["auc"] >= 0.9700:
        log.success(f"✓ AUC={metrics_cal['auc']:.4f} ≥ 0.9700，超参数是核心差异！v292 数据 + v291 超参 ≈ v291 性能")
    elif metrics_cal["auc"] >= 0.9600:
        log.info(f"△ AUC={metrics_cal['auc']:.4f} 在 0.9600-0.9700 之间，超参数有显著改善但非全部")
    else:
        log.warning(f"✗ AUC={metrics_cal['auc']:.4f} < 0.9600，超参数不是核心差异，v291 优势在数据/特征/ensemble架构")
