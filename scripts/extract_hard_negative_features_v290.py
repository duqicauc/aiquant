...
3. 合并市场环境特征
4. 输出和现有 hard_negative_feature_data_34d_v6.csv 相同格式
"""
...
def get_sample_data(conn, ts_code, start_date, end_date):
    """从 cache DB 查询单只股票在日期范围内的数据"""
    # daily_data
    df_daily = pd.read_sql(
        f"SELECT trade_date, open, high, low, close, pre_close, change, pct_chg, "
        f"vol, amount FROM daily_data "
        f"WHERE ts_code = '{ts_code}' AND trade_date BETWEEN '{start_date}' AND '{end_date}' "
    ...
    # stk_factor (注意：没有 ma5/ma10/volume_ratio)
    df_factor = pd.read_sql(
        f"SELECT trade_date, macd_dif, macd_dea, macd, "
        f"rsi_6, rsi_12, rsi_24, kdj_k, kdj_d, kdj_j "
        f"FROM stk_factor WHERE ts_code = '{ts_code}' "
    ...
    df_basic = pd.read_sql(
        f"SELECT trade_date, total_mv, circ_mv, turnover_rate, volume_ratio "
        f"FROM daily_basic WHERE ts_code = '{ts_code}' "
        f"AND trade_date BETWEEN '{start_date}' AND '{end_date}' "
        f"ORDER BY trade_date",
        conn
    )
...
def main():
    ...
    # 读取现有硬负样本的列名（用于对齐格式）
    existing_df = pd.read_csv("data/training/features/hard_negative_feature_data_34d_v6.csv", nrows=1)
    ...
    for idx, row in hn_samples.iterrows():
        ...
        start_df = pd.read_sql(
            f"SELECT trade_date FROM daily_data WHERE trade_date <= '{t1_date}' "
            f"GROUP BY trade_date ORDER BY trade_date DESC LIMIT 35",
            conn
        )
        if len(start_df) < 35:
            continue
        start_date = start_df['trade_date'].iloc[-1]

        # 查询数据
        df_raw = get_sample_data(conn, ts_code, start_date, t1_date)
