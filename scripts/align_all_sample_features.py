...
def calculate_missing_features(df: pd.DataFrame, required_features: list) -> pd.DataFrame:
    ...
    if 'volatility_vs_hist' in required_features and 'volatility_vs_hist' not in df.columns and n >= 60:
        vol_20 = df['pct_chg'].rolling(20).std() if 'pct_chg' in df.columns else df['close'].pct_change().rolling(20).std()
        vol_60 = df['pct_chg'].rolling(60).std() if 'pct_chg' in df.columns else df['close'].pct_change().rolling(60).std()
    ...
    for col in ['market_pct_chg', 'market_return_34d', 'market_volatility_34d', 
                'excess_return', 'excess_return_cumsum', 'market_trend']:
    ...
    for sample_id in sample_ids:
        ...
        try:
            ...
            if cols_to_add:
                ...
                merged = pd.merge(
                    sample_data,
                    df_with_features[['trade_date'] + cols_to_add],
                    on='trade_date',
                    how='left'
                )
