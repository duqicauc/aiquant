...
def load_ensemble_model():
    ...
    log.info(f"权重: XGBoost={weights['xgboost']:.4f}, LightGBM={weights['lightgbm']:.4f}, CatBoost={weights['catboost']:.4f}")
...
def get_valid_stocks(dm, predict_date):
    ...
    if stock_list['list_date'].dtype == 'int64':
        stock_list['list_date_dt'] = pd.to_datetime(stock_list['list_date'].astype(str), format='%Y%m%d', errors='coerce')
    else:
...
def predict_top50(predict_date: str, enable_fundamental_screening: bool = False, screening_mode: str = 'post_top50'):
    ...
    for i, row in top50.iterrows():
        ...
        log.info(f"{rank:<4} {row['ts_code']:<12} {row['name']:<10} {row['probability']:>10.4f} "
...
def main():
    ...
    parser.add_argument('dates', nargs='*', default=['20251231', '20260116'], 
                       help='预测日期列表 (YYYYMMDD)')
    parser.add_argument('--fundamental', action='store_true', 
                       help='启用基本面筛选（市值10-100亿，营收>1亿，净利润>200万，ROE>0，ROA>0）')
    parser.add_argument('--screening-mode', type=str, default='post_top50',
