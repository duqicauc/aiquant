...
OUTPUT_DIR = PROJECT_ROOT / "data" / "prediction" / "v296_stk_factor"


def main():
    parser = argparse.ArgumentParser(description="v2.9.6 批量预测")
    ...
    predictor = EnsemblePredictor(model_version="v2.9.6-ensemble")
    ...
    log.success("=" * 80)
    log.success(f"v2.9.6 预测完成: {len(results)} 天")
    log.success("=" * 80)
