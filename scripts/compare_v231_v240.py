...
def main():
    ...
    if v231_result and v240_result:
        ...
        log.info(f"{'T1前平均涨幅':<20} {v231_result['avg_pre_t1']:>+10.1f}%     {v240_result['avg_pre_t1']:>+10.1f}%     {pre_t1_diff:>+10.1f}%")
        
        # 实际收益对比
        return_diff = v240_result['avg_return'] - v231_result['avg_return']
        log.info(f"{'实际平均收益':<20} {v231_result['avg_return']:>+10.1f}%     {v240_result['avg_return']:>+10.1f}%     {return_diff:>+10.1f}%")
        
        # 胜率对比
        win_diff = v240_result['win_rate'] - v231_result['win_rate']
        log.info(f"{'胜率':<20} {v231_result['win_rate']:>10.0f}%     {v240_result['win_rate']:>10.0f}%     {win_diff:>+10.0f}%")
        ...
        if v240_result['avg_return'] > v231_result['avg_return']:
            ...
            log.warning(f"⚠️  v2.4.0实际收益较低（{v240_result['avg_return']:.1f}% vs {v231_result['avg_return']:.1f}%）")
