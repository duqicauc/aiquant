...
def load_training_data():
    ...
    exclude_cols = {'label', 'sample_id', 'ts_code', 'name', 't1_date', 't2_date', 
                    'trade_date', 'list_date', 'pattern_type', 'days_to_t1'}
...
def get_feature_columns(df, feature_cols):
    ...
    unused_binary_features = [
        'breakout_high_10d', 'breakout_high_20d', 'breakout_ma10', 'breakout_ma55',
        'high_volume_breakout', 'volume_price_match'
    ]
    exclude_cols.extend(unused_binary_features)
    
    valid_cols = [c for c in feature_cols if c not in exclude_cols 
...
def run_hyperparameter_search():
    ...
    key_combinations = [
        # 当前配置（baseline）
        {'max_depth': 6, 'learning_rate': 0.05, 'colsample_bytree': 0.6, 
         'subsample': 0.8, 'reg_alpha': 0.3, 'reg_lambda': 0.5},
        # 更深的树
        {'max_depth': 8, 'learning_rate': 0.03, 'colsample_bytree': 0.7, 
         'subsample': 0.8, 'reg_alpha': 0.5, 'reg_lambda': 1.0},
        # 更浅但更多树
        {'max_depth': 4, 'learning_rate': 0.02, 'colsample_bytree': 0.8, 
         'subsample': 0.9, 'reg_alpha': 0.1, 'reg_lambda': 0.5},
        # 强正则化
        {'max_depth': 6, 'learning_rate': 0.05, 'colsample_bytree': 0.6, 
        ...
        {'max_depth': 6, 'learning_rate': 0.1, 'colsample_bytree': 0.8, 
         'subsample': 0.9, 'reg_alpha': 0.1, 'reg_lambda': 0.5},
        # 平衡配置
        {'max_depth': 6, 'learning_rate': 0.05, 'colsample_bytree': 0.7, 
         'subsample': 0.8, 'reg_alpha': 0.3, 'reg_lambda': 1.0},
    ]
    ...
    for i, combo in enumerate(key_combinations):
        ...
        log.info(f"  max_depth={combo['max_depth']}, lr={combo['learning_rate']}, "
        ...
        log.info(f"  结果: AUC={metrics['auc']:.4f}, F1={metrics['f1']:.4f}, "
    ...
    for i, r in enumerate(results):
        ...
        log.info(f"    max_depth={p['max_depth']}, lr={p['learning_rate']}, "
                f"colsample={p['colsample_bytree']}, subsample={p['subsample']}")
        log.info(f"    reg_alpha={p['reg_alpha']}, reg_lambda={p['reg_lambda']}, "
