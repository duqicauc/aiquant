...
def add_features_inplace(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'atr_14' not in df.columns:
        ...
        df['atr_14'] = groups.apply(lambda g: safe_rolling(g[tr.name] if tr.name in g.columns else tr.loc[g.index], 14, 'mean')).values if 'sample_id' in df.columns else safe_rolling(tr, 14, 'mean')
        # 简化：直接用整体滚动
    ...
    if 'close_vs_ma10_std' not in df.columns:
        ...
        diff = df['close'] - ma10
        diff_std = groups.apply(lambda g: safe_rolling(g['close'] - ma10.loc[g.index], 10, 'std')).values if 'sample_id' in df.columns else safe_rolling(diff, 10, 'std')
    ...
    if 'days_near_ma10' not in df.columns:
        ...
        df['days_near_ma10'] = groups.apply(lambda g: safe_rolling(near_ma10.loc[g.index], 10, 'sum')).values if 'sample_id' in df.columns else safe_rolling(near_ma10, 10, 'sum')
    ...
    if 'vol_ma5_ratio' not in df.columns:
        df['vol_ma5_ratio'] = vol / (groups['vol'].transform(lambda x: safe_rolling(x, 5, 'mean')) + 1e-8) if 'sample_id' in df.columns else vol / (safe_rolling(vol, 5, 'mean') + 1e-8)
    if 'vol_ma20_ratio' not in df.columns:
        df['vol_ma20_ratio'] = vol / (groups['vol'].transform(lambda x: safe_rolling(x, 20, 'mean')) + 1e-8) if 'sample_id' in df.columns else vol / (safe_rolling(vol, 20, 'mean') + 1e-8)
    if 'volume_shrink_ratio' not in df.columns:
        vol_ma5 = groups['vol'].transform(lambda x: safe_rolling(x, 5, 'mean')) if 'sample_id' in df.columns else safe_rolling(vol, 5, 'mean')
        vol_ma20 = groups['vol'].transform(lambda x: safe_rolling(x, 20, 'mean')) if 'sample_id' in df.columns else safe_rolling(vol, 20, 'mean')
    ...
    if 'breakout_ma55' not in df.columns:
        ma55 = groups['close'].transform(lambda x: safe_rolling(x, 55, 'mean')) if 'sample_id' in df.columns else safe_rolling(df['close'], 55, 'mean')
    ...
    if 'consecutive_new_high' not in df.columns:
        ...
        df['consecutive_new_high'] = groups.apply(lambda g: safe_rolling(new_high.loc[g.index], 5, 'sum')).values if 'sample_id' in df.columns else safe_rolling(new_high, 5, 'sum')
    ...
    if 'price_down_vol_up_count_10d' not in df.columns:
        df['price_down_vol_up_count_10d'] = groups.apply(lambda g: safe_rolling(g['price_down_vol_up'], 10, 'sum')).values if 'sample_id' in df.columns else safe_rolling(df['price_down_vol_up'], 10, 'sum')
    ...
    if 'price_up_vol_down_count_10d' not in df.columns:
        df['price_up_vol_down_count_10d'] = groups.apply(lambda g: safe_rolling(g['price_up_vol_down'], 10, 'sum')).values if 'sample_id' in df.columns else safe_rolling(df['price_up_vol_down'], 10, 'sum')
    
    # 量价相关性
    for period in [10, 20]:
        col = f'volume_price_corr_{period}d'
        if col not in df.columns:
            df[col] = groups.apply(lambda g: g['close'].rolling(period, min_periods=5).corr(g['vol'])).values if 'sample_id' in df.columns else df['close'].rolling(period, min_periods=5).corr(vol)
    ...
    if 'volume_price_match_sum_10d' not in df.columns:
        df['volume_price_match_sum_10d'] = groups.apply(lambda g: safe_rolling(g['volume_price_match'], 10, 'sum')).values if 'sample_id' in df.columns else safe_rolling(df['volume_price_match'], 10, 'sum')
    
    # 量能突破
    if 'volume_breakout_count_20d' not in df.columns:
        vol_breakout = (vol > safe_rolling(vol, 20, 'mean') * 2).astype(int)
        df['volume_breakout_count_20d'] = groups.apply(lambda g: safe_rolling(vol_breakout.loc[g.index], 20, 'sum')).values if 'sample_id' in df.columns else safe_rolling(vol_breakout, 20, 'sum')
    ...
    if 'volume_rsv_20d' not in df.columns:
        ...
        df['volume_rsv_20d'] = np.where(
            vol_high > vol_low,
            (vol - vol_low) / (vol_high - vol_low + 1e-8),
            0.5
        )
    ...
    if 'price_vs_hist_mean' not in df.columns:
        df['price_vs_hist_mean'] = df['close'] / (groups['close'].transform(lambda x: safe_rolling(x, 55, 'mean')) + 1e-8)
    if 'price_vs_hist_high' not in df.columns:
        df['price_vs_hist_high'] = df['close'] / (groups['close'].transform(lambda x: safe_rolling(x, 55, 'max')) + 1e-8)
...
def main():
    ...
    positive_samples = pos_df.groupby('sample_id').agg({
        'ts_code': 'first',
        'name': 'first',
        'trade_date': 'max',
        'circ_mv': 'first'
    ...
    # 提取特征
    log.info("\n提取负样本特征（70天）...")
    negative_features = screener.extract_features(
        negative_samples_df=negative_samples,
        lookback_days=70
    )
