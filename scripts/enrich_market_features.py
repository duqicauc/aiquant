...
def enrich_with_market_features(df: pd.DataFrame, dm: DataManager, window: int = 34) -> pd.DataFrame:
    ...
    market_data = dm.get_index_daily(index_code, extended_start.strftime("%Y%m%d"), end_date.strftime("%Y%m%d"))
    ...
    market_data["market_position_20d"] = (market_data["close"] - market_low_20d) / (
...
def main():
    ...
    for file_info in files_to_process:
        ...
        results.append({"name": file_info["name"], "success": success})
