...
def write_report(results: dict, start_date: str, end_date: str, output_path: Path):
    ...
    best_return_mode = max(
        ["none", "close", "intraday_low"], key=lambda m: results.get(m, {}).get("final_return_pct") or -1e9
    )
    ...
    line_ret = (
        f"- **收益最高**：{MODE_LABELS[best_return_mode]}（{ret_val:+.2f}%）"
        if ret_val is not None
        else "- **收益最高**：—"
    )
    line_dd = (
        f"- **回撤最小**：{MODE_LABELS[best_dd_mode]}（最大回撤 {dd_val:.2f}%）"
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="输出报告路径，默认 results/compare_stop_loss_modes_*.md"
    )
    ...
    for mode in ["none", "close", "intraday_low"]:
        ...
        if r:
            log.info(
                f"  {MODE_LABELS[mode]}: 收益={r.get('final_return_pct'):+.2f}%, 最大回撤={r.get('max_drawdown'):.2f}%, 胜率={r.get('win_rate'):.2f}%"
            )
