...
def analyze_chaojie_stock(dm, ts_code, name, predict_date):
    ...
    if vol_ratio > 1.5:
        log.info(f"  ✅ 成交量放大，资金关注度高")
    elif vol_ratio < 0.8:
        log.info(f"  ⚠️  成交量萎缩，关注度下降")
    else:
        log.info(f"  ➡️  成交量正常")
    ...
    log.info(f"   如果您只有沪深主板权限，无法交易此股票")
    ...
    if price_position > 80:
        log.warning(f"  ⚠️  价格处于60日高位，追高风险较大")
    elif price_position < 20:
        log.info(f"  ✅ 价格处于60日低位，可能具备上涨空间")
    
    if ma_status.startswith("✅"):
        log.info(f"  ✅ 技术面：均线多头排列，趋势向上")
    elif ma_status.startswith("❌"):
        log.warning(f"  ❌ 技术面：均线空头排列，趋势向下")
...
def analyze_main_board_stocks(dm, df_predictions, predict_date):
    ...
    # 输出分析结果
    log.info(f"\n{'='*100}")
    log.info(f"{'排名':<4} {'代码':<12} {'名称':<10} {'综合评分':<10} {'校准概率':<10} {'当前价':<8} {'5日趋势':<10} {'均线':<8} {'建议':<20}")
    log.info(f"{'-'*100}")
    ...
    for i, (_, row) in enumerate(top3.iterrows(), 1):
        ...
        log.info(f"   💡 投资思路:")
        if row['final_score'] > 0.85:
            log.info(f"      - 模型评分极高，建议重点关注")
        if row['ma_trend'] == "多头":
            log.info(f"      - 技术面：均线多头排列，趋势向上")
        if row['vol_ratio'] > 1.2:
            log.info(f"      - 成交量放大，资金关注度高")
        if row['trend_5d'] > 5:
            log.info(f"      - 短期涨幅较大，注意追高风险")
        if row['max_drawdown_20d'] < -10:
            log.info(f"      - 近期回撤较大，注意风险控制")
        if row['breakout_strength'] == 1.0:
            log.info(f"      - 突破强度满分，可能处于突破状态")
