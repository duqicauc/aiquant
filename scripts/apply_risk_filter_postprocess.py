...
def main():
    ...
    log.info("|------|--------|------------|------|")
    log.info(
        f"| 平均收益率 | {avg_return_orig:.2f}% | {avg_return_filt:.2f}% | {avg_return_filt - avg_return_orig:+.2f}% |"
    )
    log.info(
        f"| 中位数收益 | {median_return_orig:.2f}% | {median_return_filt:.2f}% | {median_return_filt - median_return_orig:+.2f}% |"
    )
    ...
    if filtered_out:
        ...
        for _, row in df_out.sort_values("return_pct").iterrows():
            log.info(
                f"    {row['ts_code']} {row['name']}: 收益{row['return_pct']:.2f}%, 34日涨{row['return_34d_at_predict']:.1f}%, 风险[{row['risk_reasons']}]"
            )
    ...
    if len(df_high_risk) > 0:
        ...
        for _, row in df_high_risk.sort_values("risk_score").iterrows():
            log.info(
                f"  {row['ts_code']} {row['name']}: 风险系数{row['risk_score']:.2f}, 收益{row['return_pct']:.2f}%, 34日涨{row['return_34d_at_predict']:.1f}%"
            )
            if row["risk_reasons"]:
