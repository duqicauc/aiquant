...
def recommend_2stocks(predict_file, eval_file=None):
    ...
    if eval_file and Path(eval_file).exists():
        ...
        df = pd.merge(df_pred, df_eval[['ts_code', 'returns', 'max_return', 'max_drawdown']], 
                     on='ts_code', how='left')
        log.info("已合并评估结果")
    else:
        df = df_pred.copy()
    ...
    # 保存推荐结果
    output_file = PROJECT_ROOT / 'data' / 'prediction' / 'trading_plan' / f'v270_recommended_2stocks_{datetime.now().strftime("%Y%m%d")}.csv'
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    top2_output = top2[['ts_code', 'name', 'probability', 'close', 'pct_chg', 'rsi_6', 
                        'returns', 'max_return', 'max_drawdown', 'comprehensive_score']].copy()
