...
def parse_args():
    ...
    parser.add_argument('--period', type=str, default='1w',
                       help='回顾周期: 1w(1周), 2w(2周), 4w(4周), 6w(6周)')
    parser.add_argument('--prediction_date', type=str, default=None,
                       help='预测日期 (格式: YYYYMMDD)，默认最新预测')
...
def generate_review_report(prediction_date, period_weeks, df_results, analysis):
    ...
    report.append(f"\n2. 胜率统计")
    report.append(f"   - 整体胜率: {winrate['整体胜率%']:.1f}% ({winrate['盈利股票数']}/{winrate['盈利股票数'] + winrate['亏损股票数']})")
    ...
    for stat in analysis['分层表现']:
        report.append(
            f"{stat['概率区间']:<12} {stat['数量']:<8} "
            f"{stat['胜率%']:<11.1f}% {stat['平均收益%']:<11.2f}%"
        )
