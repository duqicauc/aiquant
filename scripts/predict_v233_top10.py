...
def analyze_market_environment(dm, predict_date):
    ...
    try:
        ...
        log.info(f"📊 市场环境: {trend}, 动量: {momentum:.2f}, 5日涨幅: {returns_5d:.2f}%, 10日涨幅: {returns_10d:.2f}%, 量比: {vol_ratio:.2f}")
...
def analyze_sector_heat(dm, stock_list, predict_date):
    ...
    try:
        ...
        for industry, data in sorted_sectors[:10]:
            log.info(f"{industry:<15} {data['heat_score']:<8.2f} {data['return_20d']:>9.2f}% {data['return_10d']:>9.2f}% {data['return_5d']:>9.2f}% {data['vol_ratio']:<8.2f}")
...
def calculate_v233_score(cal_prob, expected_return_score, pct_chg, rsi_6, amount, 
                         consecutive_limit_up, industry, sector_heat, market_trend):
    """
...
def process_single_stock(dm, ts_code, name, industry, predict_date, feature_names, 
                        booster, calibrator, sector_heat, market_trend):
    """处理单只股票"""
    try:
        end_date = predict_date
        ...
        final_score, penalty, penalty_reasons = calculate_v233_score(
            cal_prob, expected_return_norm, pct_chg, rsi_6, amount, 
            consecutive_limit_up, industry, sector_heat, market_trend
        )
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for idx, row in valid.iterrows():
            ...
            future = executor.submit(
                ...
                dm, row['ts_code'], row['name'], industry, predict_date,
                feature_names, booster, calibrator, sector_heat, market_trend
            )
