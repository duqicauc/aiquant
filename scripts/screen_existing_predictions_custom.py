...
def screen_predictions(prediction_file: str, trade_date: str, market_cap_max: int = 100):
    ...
    """
    log.info("="*80)
    log.info(f"对已有预测结果进行基本面筛选（自定义市值上限）")
    log.info("="*80)
    ...
    # 保存通过筛选的股票
    if len(passed) > 0:
        output_file = output_dir / f"{Path(prediction_file).stem}_fundamental_screened_{market_cap_max}亿_{trade_date}.csv"
...
def main():
    ...
    parser.add_argument('--market-cap-max', type=int, default=200,
                       help='市值上限（单位：亿），默认200亿。可选值：100, 200, 300, 500等')
