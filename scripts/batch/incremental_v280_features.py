...
使用 v6 特征工程链路为新增正样本生成完整特征，截取 34 天后追加到 v5 enhanced。

流程：
1. 扫描新增正样本（2025-12-27 ~ 2026-04-14）
2. extract_features(lookback_days=70) 获取基础特征
3. enrich_features_v6.py 计算高级特征（165列）
4. 截取 days_to_t1 ∈ [-34, -1] 的行
5. 添加 13 个增强特征
6. 追加到 v5 enhanced 文件
"""
...
# ==================== 增强特征（与 feature_analysis_and_enhancement.py 一致）====================
...
def main():
    ...
    # 4. 分配新的 sample_id
    df_v5 = pd.read_csv(PROJECT_ROOT / "data" / "training" / "processed" / "feature_data_34d_v5.csv", usecols=["sample_id"])
    max_existing_id = int(df_v5["sample_id"].max())
    ...
    # 5. 提取 70 天基础特征（为 v6 特征工程准备足够数据）
    log.info("\n提取新增样本的70天基础特征...")
    df_features_70d = screener.extract_features(df_new, lookback_days=70)
    ...
    # 6.5 补充缺失的基础列（如 turnover_rate）
    log.info("\n补充缺失的基础列...")
    missing_base_cols = [c for c in ["turnover_rate", "pre_close", "change", "amount"] if c not in df_features_70d.columns]
    ...
    # 运行 add_features_inplace 补充更多特征
    ...
    # 13. 同步更新 positive_samples.csv
    df_all_samples = pd.concat([df_existing, df_new], ignore_index=True)
    df_all_samples.to_csv(existing_samples_file, index=False)
    log.success(f"✓ 正样本列表已更新: {len(df_all_samples)} 条")

    # 14. 统计
    log.info("\n" + "=" * 80)
    log.info("增量更新统计")
    log.info("=" * 80)
    log.info(f"新增正样本: {len(df_new)} 个")
