...
为新增正样本生成对应的硬负样本，提取完整特征后追加到 v5 enhanced 硬负样本文件。

流程：
1. 读取现有正/硬负样本列表
2. 确定新增正样本（2025-12-27 ~ 2026-04-14）
3. 用 HardNegativeSampleScreener 生成硬负样本
...
def main():
    ...
    existing_hard_file = PROJECT_ROOT / "data" / "training" / "samples" / "hard_negative_samples.csv"

    df_existing_pos = pd.read_csv(existing_pos_file)
    log.info(f"现有正样本: {len(df_existing_pos)} 条")

    if existing_hard_file.exists():
        df_existing_hard = pd.read_csv(existing_hard_file)
        log.info(f"现有硬负样本列表: {len(df_existing_hard)} 条")
    else:
        df_existing_hard = pd.DataFrame()
        log.warning("未找到现有硬负样本列表文件")
    ...
    if df_new_pos.empty:
        log.info("无新增正样本，跳过硬负样本生成")
        return

    # 3. 生成硬负样本
    log.info("\n生成硬负样本...")
    df_new_hard = screener.screen_hard_negatives(
    ...
    log.info(f"生成硬负样本: {len(df_new_hard)} 条")

    if df_new_hard.empty:
        log.error("硬负样本生成失败")
        return

    # 4. 分配新的 sample_id
    enhanced_file = PROJECT_ROOT / "data" / "training" / "enhanced" / "hard_negative_feature_data_34d_v5_enhanced.csv"
    ...
    df_new_hard = df_new_hard.reset_index(drop=True)
    df_new_hard["sample_id"] = range(max_existing_id + 1, max_existing_id + 1 + len(df_new_hard))
    log.info(f"新 sample_id 范围: {max_existing_id + 1} ~ {max_existing_id + len(df_new_hard)}")

    # 5. 提取 70 天基础特征
    log.info("\n提取新增硬负样本的70天基础特征...")
    df_features_70d = screener.extract_features(df_new_hard, lookback_days=70)
    ...
    v5_file = PROJECT_ROOT / "data" / "training" / "features" / "hard_negative_feature_data_34d_v5.csv"
    if v5_file.exists():
    ...
    for df_save in [df_existing_features, df_combined, df_all_hard]:
        if "trade_date" in df_save.columns:
            df_save["trade_date"] = pd.to_datetime(df_save["trade_date"]).dt.strftime("%Y-%m-%d")

    backup_file = enhanced_file.parent / f"hard_negative_feature_data_34d_v5_enhanced_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    if enhanced_file.exists():
    ...
    # 12. 同步更新硬负样本列表
    df_all_hard = pd.concat([df_existing_hard, df_new_hard], ignore_index=True)
    df_all_hard.to_csv(existing_hard_file, index=False)
    log.success(f"✓ 硬负样本列表已更新: {len(df_all_hard)} 条")

    # 13. 统计
    log.info("\n" + "=" * 80)
    log.info("增量硬负样本更新统计")
    log.info("=" * 80)
    log.info(f"新增硬负样本: {len(df_new_hard)} 个")
