...
示例:
  ...
  python scripts/generate_test_template.py src/utils/ --recursive
    ...
    parser.add_argument("source", type=Path, help="源代码文件路径（相对于项目根目录）")
    parser.add_argument("--output", "-o", type=Path, help="测试文件输出路径（可选，默认自动生成）")
    parser.add_argument("--recursive", "-r", action="store_true", help="递归处理目录中的所有Python文件")
