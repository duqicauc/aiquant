...
def add_technical_factors_to_sample(
    dm: DataManager, ts_code: str, start_date: str, end_date: str, lookback_days: int = 34
) -> pd.DataFrame:
    """
    为单个样本添加新技术因子
    """
    ...
    try:
        df_basic = dm.fetcher.pro.daily_basic(
            ts_code=ts_code, start_date=extended_start, end_date=end_date, fields="ts_code,trade_date,turnover_rate_f"
        )
        if df_basic is not None and not df_basic.empty:
            df_basic["trade_date"] = pd.to_datetime(df_basic["trade_date"])
            df_daily = pd.merge(df_daily, df_basic[["trade_date", "turnover_rate_f"]], on="trade_date", how="left")
...
def add_technical_factors_to_features(
    features_df: pd.DataFrame, dm: DataManager, lookback_days: int = 34
) -> pd.DataFrame:
    """
    为特征数据添加新技术因子
    """
    ...
    for i, sample_id in enumerate(sample_ids):
        ...
        try:
            # 获取新技术因子
            new_factors = add_technical_factors_to_sample(dm, ts_code, start_date, end_date, lookback_days)
            ...
            # 合并新因子
            sample_augmented = pd.merge(sample_data, new_factors, on="trade_date", how="left")
