...
def add_market_factors_to_features(features_df: pd.DataFrame, fetcher, window: int = 34) -> pd.DataFrame:
    """
    为特征数据添加市场因子
    ...
    if "ts_code" in result.columns:
        ...
        # 价格相对历史均值
        result["price_vs_hist_mean"] = (
            result.groupby("ts_code")
            .apply(lambda x: (x["close"] / x["close"].rolling(window=lookback, min_periods=20).mean() - 1) * 100)
            .reset_index(level=0, drop=True)
        )

        # 价格相对历史高点
        result["price_vs_hist_high"] = (
            result.groupby("ts_code")
            .apply(lambda x: (x["close"] / x["close"].rolling(window=lookback, min_periods=20).max() - 1) * 100)
            .reset_index(level=0, drop=True)
        )
...
def main():
    ...
    if os.path.exists(POSITIVE_FEATURES_FILE):
        ...
        df_pos_with_market = add_market_factors_to_features(df_pos, dm.fetcher, window=34)
    ...
    if os.path.exists(NEGATIVE_FEATURES_FILE):
        ...
        df_neg_with_market = add_market_factors_to_features(df_neg, dm.fetcher, window=34)
