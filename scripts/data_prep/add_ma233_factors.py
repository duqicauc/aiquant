...
def calculate_ma233_factors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if n >= 233:
        ...
        df["ma5_golden_cross_233"] = (
            (df["ma_5d"] > df["ma_233d"]) & (df["ma_5d"].shift(1) <= df["ma_233d"].shift(1))
        ).astype(int)
...
def process_sample_for_ma233(
    dm: DataManager, ts_code: str, sample_dates: list, lookback_days: int = 280
) -> pd.DataFrame:
    """
    为单个样本计算MA233特征
...
def add_ma233_to_features(input_file: str, output_file: str, dm: DataManager, batch_size: int = 100):
    """
    为特征文件添加MA233相关特征
    """
    log.info("=" * 80)
    ...
    for i in range(0, len(all_sample_ids), batch_size):
        ...
        for sample_id in batch_ids:
            ...
            if ma233_data is not None and not ma233_data.empty:
                # 合并
                merged = pd.merge(sample_data, ma233_data, on="trade_date", how="left")
