...
def process_sample_batch(dm: DataManager, features_df: pd.DataFrame, sample_ids: list, max_lookback: int = 55) -> list:
    """
    批量处理样本
    """
    results = []
...
def main():
    ...
    if os.path.exists(pos_output):
        ...
        log.info("\n[步骤2] 处理正样本特征...")
        add_advanced_factors_with_checkpoint(str(pos_input), str(pos_output), str(pos_checkpoint), dm)
    ...
    if os.path.exists(neg_output):
        ...
        log.info("\n[步骤3] 处理负样本特征...")
        add_advanced_factors_with_checkpoint(str(neg_input), str(neg_output), str(neg_checkpoint), dm)
