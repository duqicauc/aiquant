...
def add_advanced_factors_to_sample(
    dm: DataManager, ts_code: str, start_date: str, end_date: str, max_lookback: int = 55
) -> pd.DataFrame:
    """
    为单个样本添加高级技术因子
    """
    try:
        ...
        # 支撑阻力
        sr_cols = [
            c for c in df_daily.columns if "support" in c or "resistance" in c or "dist_to" in c or "channel" in c
        ]
        ...
        vol_adv_cols = [
            ...
            if "volume_trend" in c
            or "volume_breakout_count" in c
            or "price_up_vol" in c
            or "price_down_vol" in c
            or "volume_rsv" in c
            or "obv" in c
        ]
...
def add_advanced_factors_to_features(
    features_df: pd.DataFrame, dm: DataManager, max_lookback: int = 55
) -> pd.DataFrame:
    """
    为特征数据添加高级技术因子
    """
