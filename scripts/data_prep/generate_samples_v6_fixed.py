...
def extract_sample_features(
    dm: DataManager, ts_code: str, name: str, t1_date: str, lookback_days: int, sample_id: int
) -> pd.DataFrame:
    """
...
def calculate_advanced_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def calc_sample_features(g):
        ...
        if "atr_14" not in g.columns and n >= 14:
            tr = pd.concat([high - low, (high - close.shift(1)).abs(), (low - close.shift(1)).abs()], axis=1).max(
                axis=1
            )
...
def generate_positive_samples_v6_fixed():
    ...
    # 获取唯一样本列表
    samples = df_v5.groupby("sample_id").agg({"ts_code": "first", "name": "first", "trade_date": "max"}).reset_index()
    ...
    for idx, sample in samples.iterrows():
        ...
        try:
            features = extract_sample_features(
                dm, sample["ts_code"], sample["name"], sample["t1_date"], LOOKBACK_DAYS, idx
            )
