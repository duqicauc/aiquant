...
def calculate_anti_chasing_features(df):
    ...
    if 'high_34d' in df.columns and 'low_34d' in df.columns:
        df['price_range_pct'] = np.where(
            df['low_34d'] > 0,
            (df['high_34d'] - df['low_34d']) / df['low_34d'] * 100,
            0
        )
    else:
    ...
    if 'vol_ma5_ratio' in df.columns and 'vol_ma20_ratio' in df.columns:
        df['volume_shrink_ratio'] = np.where(
            df['vol_ma20_ratio'] > 0,
            df['vol_ma5_ratio'] / df['vol_ma20_ratio'],
            1
        )
    else:
...
def main():
    ...
    # 显示新增特征统计
    new_features = ['price_range_pct', 'close_vs_ma10_std', 'days_near_ma10', 
