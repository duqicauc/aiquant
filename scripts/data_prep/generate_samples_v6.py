...
def generate_positive_samples_v6():
    ...
    # 提取特征（使用70天）
    log.info("\n开始提取正样本特征...")
    positive_features = screener.extract_features(
        samples_df=samples,
...
def generate_negative_samples_v6(positive_df: pd.DataFrame):
    ...
    positive_samples = positive_df.groupby('sample_id').agg({
        'ts_code': 'first',
        'name': 'first',
        'trade_date': 'max',  # 取最后一天
        'circ_mv': 'first'
    }).reset_index()
...
def generate_hard_negative_samples_v6(positive_df: pd.DataFrame):
    ...
    hard_negative_features = screener.extract_features(
        hard_negative_samples_df=hard_negative_samples,
        lookback_days=LOOKBACK_DAYS
    )
    ...
    if 'circ_mv' in df_pos.columns and 'circ_mv' in df_neg.columns:
        ...
        if len(pos_mv) > 0 and len(neg_mv) > 0:
            ...
            if abs(bias) < 30:
                ...
                log.warning(f"  ⚠ 市值偏差仍较大，建议进一步优化")
    
    # 硬负样本比例检查
    if hard_ratio >= 15:
        log.success(f"  ✓ 硬负样本比例达标 (目标>=15%)")
    else:
        log.warning(f"  ⚠ 硬负样本比例未达标 (目标>=15%)")
