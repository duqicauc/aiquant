...
def calculate_all_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def calc_sample_features(g):
        ...
        if 'high' not in g.columns:
            log.warning(f"样本 {g['sample_id'].iloc[0] if 'sample_id' in g.columns else 'unknown'}: 缺少high列，使用pct_chg估算")
        ...
        if 'recovery_ratio_20d' not in g.columns and n >= 20:
            ...
            g['recovery_ratio_20d'] = np.where(
                ...
                (close - rolling_min) / (rolling_max - rolling_min + 1e-8),
                0.5
            )
        ...
        if 'volume_price_divergence' not in g.columns and n >= 10:
            ...
            vol_trend = (vol.rolling(5).mean() - vol.rolling(10).mean().shift(5)) / (vol.rolling(10).mean().shift(5) + 1e-8)
            g['volume_price_divergence'] = ((price_trend > 0) & (vol_trend < 0) | (price_trend < 0) & (vol_trend > 0)).astype(int)
...
def main():
    ...
    target_cols = [c for c in df_v5.columns if c not in ['sample_id', 'ts_code', 'name', 'trade_date', 'label', 'days_to_t1']]
    ...
    df_hard = pd.read_csv(PROJECT_ROOT / 'data' / 'training' / 'features' / 'hard_negative_feature_data_34d_v6.csv', nrows=1)
