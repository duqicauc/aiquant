...
def calculate_all_optimized_factors(df: pd.DataFrame, market_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    if n >= 233:
        ...
        df["breakout_ma233_first"] = (
            (df["close"] > df["ma_233d"]) & (df["close"].shift(1) <= df["ma_233d"].shift(1))
        ).astype(
            int
        )  # 首次突破
        ...
        if "ma5" in df.columns:
            ...
            df["ma5_cross_ma233"] = (
                (df["ma5"] > df["ma_233d"]) & (df["ma5"].shift(1) <= df["ma_233d"].shift(1))
            ).astype(
                int
            )  # 5日线上穿233日线
    ...
    for period in [10, 20, 55, 233]:
        if n >= period:
            ...
            df[f"breakout_pct_{period}d"] = np.where(
                df["close"] > prev_high, (df["close"] - prev_high) / prev_high * 100, 0
            )
    ...
    for period in [20, 55]:
        if n >= period + 3:
            ...
            df[f"breakout_valid_{period}d"] = (df["low"].rolling(3).min() > breakout_price.ffill()).astype(int)
    ...
    if "vol" in df.columns and n >= 20:
        ...
        # 3.1 经典量价形态识别
        # 放量上涨（健康上涨信号）
        df["vol_up_rise"] = np.where((price_pct > 2) & (vol_ratio > 1.5), vol_ratio, 0)  # 放量程度
        df["vol_up_rise_count_10d"] = (df["vol_up_rise"] > 0).rolling(10).sum()

        # 放量不涨（警示信号）
        df["vol_up_flat"] = np.where((price_pct >= -2) & (price_pct <= 2) & (vol_ratio > 1.5), vol_ratio, 0)
        df["vol_up_flat_count_10d"] = (df["vol_up_flat"] > 0).rolling(10).sum()

        # 缩量下跌（洗盘信号）
        df["vol_down_drop"] = np.where((price_pct < -2) & (vol_ratio < 0.7), 1, 0)
        df["vol_down_drop_count_10d"] = df["vol_down_drop"].rolling(10).sum()

        # 缩量不涨（蓄势信号）
        df["vol_down_flat"] = np.where((price_pct >= -2) & (price_pct <= 2) & (vol_ratio < 0.7), 1, 0)
        df["vol_down_flat_count_10d"] = df["vol_down_flat"].rolling(10).sum()

        # 放量下跌（危险信号）
        df["vol_up_drop"] = np.where((price_pct < -2) & (vol_ratio > 1.5), vol_ratio, 0)
        ...
        df["bottom_consolidation"] = np.where((price_range_10d < 10) & (price_position < 0.3), 1, 0)
        df["bottom_consolidation_days"] = df["bottom_consolidation"].rolling(20).sum()

        # 缩量盘整
        vol_shrink = vol_ratio < 0.8
        df["vol_shrink_consolidation"] = np.where((price_range_10d < 10) & vol_shrink, 1, 0)
        df["vol_shrink_consolidation_days"] = df["vol_shrink_consolidation"].rolling(20).sum()

        # 3.3 健康量价得分
        df["healthy_vol_price_score"] = (
            df["vol_up_rise_count_10d"] * 2
            + df["vol_down_drop_count_10d"] * 1  # 放量上涨（权重2）
            - df["vol_up_drop_count_10d"] * 2  # 缩量下跌（权重1）
            - df["vol_up_flat_count_10d"] * 1  # 放量下跌（权重-2）  # 放量不涨（权重-1）
        ) / 10.0  # 归一化
        ...
        if "amount" in df.columns:
            ...
            df["money_flow_ratio_10d"] = pd.Series(up_amount).rolling(10).sum() / (
                df["amount"].rolling(10).sum() + 1e-8
            )

        # 3.6 放量突破确认
        breakout_20d = df["close"] > df["close"].rolling(20).max().shift(1)
        df["vol_breakout_confirm"] = np.where(breakout_20d & (vol_ratio > 1.5), vol_ratio, 0)
    ...
    if "kdj_k" in df.columns:
        df["kdj_golden_cross"] = ((df["kdj_k"] > df["kdj_d"]) & (df["kdj_k"].shift(1) <= df["kdj_d"].shift(1))).astype(
            int
        )
        df["kdj_death_cross"] = ((df["kdj_k"] < df["kdj_d"]) & (df["kdj_k"].shift(1) >= df["kdj_d"].shift(1))).astype(
            int
        )
    ...
    if "pct_chg" in df.columns:
        ...
        if all(f"bias_{p}d" in df.columns for p in [5, 10, 20]):
            df["high_bias_risk"] = ((df["bias_5d"] > 10) | (df["bias_10d"] > 15) | (df["bias_20d"] > 20)).astype(int)

        # 5.3 技术指标超买风险
        rsi_overbought = (df["rsi_6"] > 80).astype(int) if "rsi_6" in df.columns else pd.Series([0] * n)
        kdj_overbought = (
            ((df["kdj_k"] > 80) & (df["kdj_d"] > 80)).astype(int)
        ...
        df["tech_overbought_risk"] = (rsi_overbought + kdj_overbought + cci_overbought) / 3.0
        ...
        df["chasing_risk_level"] = pd.cut(
            df["chasing_risk_score"], bins=[-0.01, 0.3, 0.6, 1.01], labels=[0, 1, 2]
        ).astype(int)
        ...
        if "close" in df.columns and "market_close" in df.columns:
            ...
            if n >= 20:
                ...
                df["relative_strength_20d"] = np.where(
                    market_return_20d != 0, stock_return_20d / (market_return_20d + 1e-8), 1
                )
    ...
    for tf in [8, 34, 55]:
        if n >= tf:
            ...
            if f"volatility_{tf}d" not in df.columns:
                df[f"volatility_{tf}d"] = (
                    df["pct_chg"].rolling(tf).std()
                    if "pct_chg" in df.columns
                    else df["close"].pct_change().rolling(tf).std()
                )
...
def process_sample_batch(
    ...
    for sample_id in sample_ids:
        ...
        try:
            ...
            if date_key not in market_data_cache:
                try:
                    ...
                    if market_df is not None and not market_df.empty:
                        market_df["trade_date"] = pd.to_datetime(market_df["trade_date"])
                        # 重命名列以便合并
                        market_df = market_df.rename(columns={"close": "market_close", "pct_chg": "market_pct_chg"})
            ...
            merged = pd.merge(sample_data, df_with_factors[["trade_date"] + new_cols], on="trade_date", how="left")
...
def add_optimized_factors_with_checkpoint(
    input_file: str, output_file: str, checkpoint_file: str, dm: DataManager, batch_size: int = 100
):
    """
    带断点续传的优化因子添加
    """
...
def main():
    ...
    if os.path.exists(pos_output):
        ...
        log.info("\n[步骤2] 处理正样本优化特征...")
        add_optimized_factors_with_checkpoint(str(pos_input), str(pos_output), str(pos_checkpoint), dm)
    ...
    if os.path.exists(neg_output):
        ...
        log.info("\n[步骤3] 处理负样本优化特征...")
        add_optimized_factors_with_checkpoint(str(neg_input), str(neg_output), str(neg_checkpoint), dm)
