            ...
            try:
                ...
                for i in range(len(df) - 30):  # 需要至少30天后续数据
                    ...
                    if is_similar_pattern:
                        ...
                        is_failed = (
                            (max_drawdown < -15) or      # 回撤超过15%
                            (max_gain < 20) or           # 最大涨幅<20%
                            (final_return < 5)           # 最终涨幅<5%
                        )
                        ...
                        if is_failed:
                            batch_samples.append({
                                ...
                                'ts_code': ts_code,
                                'name': stock['name'],
                                't1_date': row['trade_date'].strftime('%Y%m%d') if hasattr(row['trade_date'], 'strftime') else str(row['trade_date']),
...
def main():
    ...
    df_false_breakouts = find_false_breakout_samples(
        dm, start_date, end_date_extended, positive_stocks_set,
