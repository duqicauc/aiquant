...
def analyze_stock_score_trend(ts_code, stock_name, start_date, end_date):
    ...
    # 重命名price_df的列以避免冲突
    price_df_renamed = price_df[['trade_date_str', 'close', 'pct_chg']].rename(columns={
        'close': 'close_actual',
        'pct_chg': 'pct_chg_actual'
    })
    ...
    print("详细数据表")
    print("="*100)
    print(f"{'日期':<12} {'收盘价':<10} {'当日涨跌':<10} {'模型评分':<12} {'排名':<8} {'排名分位':<10} {'未来1日':<10} {'未来3日':<10} {'未来5日':<10} {'累计收益':<10}")
    print("-"*100)
    
    for _, row in merged_df.iterrows():
        close_actual = row['close_actual'] if pd.notna(row['close_actual']) else row.get('close', 0)
        pct_chg = row.get('pct_chg_actual', row.get('pct_chg', 0)) if pd.notna(row.get('pct_chg_actual', row.get('pct_chg', 0))) else 0
    ...
    print(f"  最高评分: {merged_df['final_score'].max():.4f} (日期: {merged_df.loc[merged_df['final_score'].idxmax(), 'date'].strftime('%Y-%m-%d')})")
    print(f"  最低评分: {merged_df['final_score'].min():.4f} (日期: {merged_df.loc[merged_df['final_score'].idxmin(), 'date'].strftime('%Y-%m-%d')})")
    print(f"  评分标准差: {merged_df['final_score'].std():.4f}")
    
    print(f"\n排名统计:")
    print(f"  平均排名: {merged_df['rank'].mean():.0f}")
    print(f"  最佳排名: {merged_df['rank'].min()} (日期: {merged_df.loc[merged_df['rank'].idxmin(), 'date'].strftime('%Y-%m-%d')})")
    print(f"  最差排名: {merged_df['rank'].max()} (日期: {merged_df.loc[merged_df['rank'].idxmax(), 'date'].strftime('%Y-%m-%d')})")
    ...
    print(f"  期间涨跌: {(merged_df['close_actual'].iloc[-1] - merged_df['close_actual'].iloc[0]) / merged_df['close_actual'].iloc[0] * 100:+.2f}%")
    print(f"  最高价格: {merged_df['close_actual'].max():.2f} (日期: {merged_df.loc[merged_df['close_actual'].idxmax(), 'date'].strftime('%Y-%m-%d')})")
    print(f"  最低价格: {merged_df['close_actual'].min():.2f} (日期: {merged_df.loc[merged_df['close_actual'].idxmin(), 'date'].strftime('%Y-%m-%d')})")
