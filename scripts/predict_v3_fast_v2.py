#!/usr/bin/env python3
"""
v3.0.0 批量快速预测 v2（基于 ArcticDB + 预计算特征）

核心优化：
1. 预先为所有股票计算完整日期范围的特征（只做一次 compute_all_features）
2. 每个预测日期从预计算特征中截取 34 天窗口
3. 避免 78 次重复的特征计算

Usage:
    python scripts/predict_v3_fast_v2.py --start 20260105 --end 20260508
"""
import argparse
import json
import sys
...
def predict_range_fast(start_date: str, end_date: str):
    """基于 ArcticDB 的批量快速预测（预计算特征版）"""
    log.info(f"{'='*60}")
    log.info(f"V3.0.0 快速预测 v2: {start_date} ~ {end_date}")
    log.info(f"{'='*60}")
    ...
    # 3. 预计算所有股票的完整特征时间序列（只做一次！）
    log.info(f"{'='*60}")
    log.info("预计算所有股票特征时间序列...")
    log.info(f"{'='*60}")

    # 按股票分组，分别计算特征，然后合并
    all_features = []
    grouped = df_raw.groupby("ts_code")
    total = len(grouped)
    ...
    for i, (ts_code, group) in enumerate(grouped, 1):
        ...
        if not df_market.empty:
            mkt = df_market[
                (df_market["trade_date"] >= group["trade_date"].min())
                & (df_market["trade_date"] <= group["trade_date"].max())
            ].copy()
        else:
            mkt = pd.DataFrame()

        try:
            df_feat = engineer.compute_all_features(group, mkt)
        except Exception as e:
            log.debug(f"  {ts_code} 特征计算失败: {e}")
            continue

        df_feat["ts_code"] = ts_code
        all_features.append(df_feat)

        if i % 500 == 0 or i == total:
            log.info(f"  已处理 {i}/{total} 只股票...")
    ...
    df_all_features["trade_date"] = pd.to_datetime(df_all_features["trade_date"])
    log.success(f"预计算完成: {len(df_all_features)} 行, {df_all_features['ts_code'].nunique()} 只股票")
    ...
    for i, pred_date in enumerate(pred_dates, 1):
        ...
        # 从预计算特征中截取窗口
        df_window = df_all_features[
            (df_all_features["trade_date"] >= lookback_start) & (df_all_features["trade_date"] <= lookback_end)
        ].copy()
        ...
        sample_features = []
        for ts_code, group in df_window.groupby("ts_code"):
            group = group.sort_values("trade_date").tail(LOOKBACK_DAYS)
            if len(group) < LOOKBACK_DAYS * 0.7:
                continue
            group["sample_id"] = ts_code
            group["name"] = ""
            group["days_to_t1"] = range(-len(group), 0)
            group["label"] = 0
            group["trade_date"] = pred_date  # T1 日期
            sample_features.append(group)
...
def main():
    parser = argparse.ArgumentParser(description="v3.0.0 快速批量预测 v2")
