...
def load_complementary_top10_per_day(
    ...
    for date in trading_days:
        ...
        sort_col = 'sort_key' if 'sort_key' in df.columns else ('dual_score' if 'dual_score' in df.columns else 'final_score')
...
def prepare_stock_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['pct_3d'] = np.where(
        df['close_3d_ago'].notna() & (df['close_3d_ago'] > 0),
        (df['close'] / df['close_3d_ago'] - 1) * 100,
        np.nan
    )
    return df
...
def main():
    ...
    out_path = args.output or (PROJECT_ROOT / 'data' / 'prediction' / 'results' / f'top10_first_meet_conditions_{args.start}_{args.end}.csv')
