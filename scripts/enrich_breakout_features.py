...
def calculate_breakout_features_with_history(
    ...
    if n >= 20 and 'vol' in combined.columns:
        ...
        combined['breakout_volume_strength'] = np.where(
            breakout_20d == 1,
            combined['vol'] / (vol_ma20 + 1e-8),
            0
        )
    else:
    ...
    new_feature_cols = [
        ...
        'breakout_volume_strength', 'breakout_confirmed_10d', 'breakout_confirmed_20d',
        'breakout_resonance'
    ]
...
def _calculate_breakout_for_sample(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if n < 10:
        for col in ['breakout_strength_10d', 'breakout_strength_20d', 'breakout_strength_55d',
                    'breakout_volume_strength', 'breakout_confirmed_10d', 'breakout_confirmed_20d',
                    'breakout_resonance']:
            df[col] = np.nan
        return df
...
def process_file(input_file: Path, dm: DataManager) -> bool:
    ...
    # 检查是否已有突破特征
    breakout_cols = ['breakout_strength_10d', 'breakout_strength_20d', 'breakout_strength_55d',
                     'breakout_volume_strength', 'breakout_confirmed_10d', 'breakout_confirmed_20d',
                     'breakout_resonance']
    ...
    if existing_cols:
        ...
        log.info(f"已删除旧的突破特征列")
    
    # 转换日期格式
    df['trade_date'] = pd.to_datetime(df['trade_date'])
    
    # 获取每个样本的信息
    sample_info = df.groupby('sample_id').agg({
        'ts_code': 'first',
        'trade_date': 'max'  # 样本最后一天
    }).reset_index()
    
    samples_list = [
        (row['sample_id'], row['ts_code'], row['trade_date'].strftime('%Y-%m-%d'))
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for sample_id, ts_code, end_date in batch_samples:
                future = executor.submit(
                    fetch_extended_data, dm, ts_code, end_date, 80
                )
