...
# -*- coding: utf-8 -*-
"""
v2.9.4 批量预测脚本（专业级集成版）

使用 v2.9.4-ensemble 模型，含温度缩放校准 + Diversity-aware 加权平均。
"""
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "prediction" / "v294_stk_factor"


def main():
    parser = argparse.ArgumentParser(description="v2.9.4 批量预测")
    ...
    predictor = EnsemblePredictor(model_version="v2.9.4-ensemble")
    ...
    log.success("=" * 80)
    log.success(f"v2.9.4 预测完成: {len(results)} 天")
    log.success("=" * 80)
