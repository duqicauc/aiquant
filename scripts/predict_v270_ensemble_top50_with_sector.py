...
def predict_top50(predict_date: str):
    ...
    for idx, (_, row) in enumerate(stock_list.iterrows()):
        ...
        result = process_single_stock(
            dm, row['ts_code'], row['name'], row.get('industry', '未知'),
            predict_date, feature_names, models, weights
        )
    ...
    for i, row in top50.iterrows():
        ...
        log.info(f"{rank:<4} {row['ts_code']:<12} {row['name']:<10} {row['industry']:<15} {row['probability']:>10.4f} "
