...
def fix_negative_sample_features(
    dm: DataManager, input_file: Path, output_file: Path, checkpoint_file: Path, batch_size: int = 100
):
    """
    修复负样本缺失的特征（带断点续传）
    """
        ...
        for sample_id in batch_ids:
            ...
            try:
                ...
                if cols_to_add:
                    # 合并补充数据
                    merged = pd.merge(
                        sample_data, df_complete[["trade_date"] + cols_to_add], on="trade_date", how="left"
                    )
...
def main():
    ...
    # 修复特征
    log.info("\n[步骤2] 修复缺失特征...")
    fix_negative_sample_features(dm, input_file, output_file, CHECKPOINT_FILE, BATCH_SIZE)
