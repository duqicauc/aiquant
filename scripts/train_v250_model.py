...
注意：
- 统一使用v5版本文件
- 训练前需先运行 scripts/align_all_sample_features.py 确保特征对齐
"""
...
def load_training_data():
    ...
    文件路径固定（需先运行 align_all_sample_features.py）：
    ...
    if missing_files:
        ...
        for f in missing_files:
            log.error(f"  - {f}")
        log.error("请先运行: python scripts/align_all_sample_features.py")
        raise FileNotFoundError("v5版本文件不完整")
...
def time_series_split(df, test_size=0.2, cal_size=0.15):
    ...
    if len(df) == 0:
        log.error(f"日期解析后数据为空，请检查日期列格式")
    ...
    log.info(f"\n时间序列划分:")
    log.info(f"  训练集: {train[date_col].min().date()} ~ {train[date_col].max().date()} ({len(train)}条, 正:{train['label'].sum()}, 负:{len(train)-train['label'].sum()})")
    log.info(f"  校准集: {cal[date_col].min().date()} ~ {cal[date_col].max().date()} ({len(cal)}条, 正:{cal['label'].sum()}, 负:{len(cal)-cal['label'].sum()})")
    log.info(f"  测试集: {test[date_col].min().date()} ~ {test[date_col].max().date()} ({len(test)}条, 正:{test['label'].sum()}, 负:{len(test)-test['label'].sum()})")
...
def save_model(booster, calibrator, feature_names, metrics):
    ...
    metadata = {
        ...
        'description': 'v2.6.0模型 - 参数调优（colsample_bytree=0.6, max_depth=6, reg减少），含增强市场环境特征+突破强度特征+特征交互项+风险特征+概率校准+时间序列划分',
...
def main():
    log.info("="*80)
    log.info("训练v2.6.0模型（参数调优+增强市场环境特征+突破强度特征+特征交互项）")
    log.info("="*80)
