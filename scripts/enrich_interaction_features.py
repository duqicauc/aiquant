...
def calculate_interaction_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'trend_slope_8d' in df.columns and 'trend_slope_34d' in df.columns:
        df['trend_consistency'] = (
            np.sign(df['trend_slope_8d']) == np.sign(df['trend_slope_34d'])
        ).astype(int)
    ...
    if 'pct_chg' in df.columns:
        if 'volume_change' in df.columns:
            df['volume_price_divergence'] = (
                np.sign(df['pct_chg']) != np.sign(df['volume_change'])
            ).astype(int)
            log.info("  ✓ 计算 volume_price_divergence")
        elif 'vol' in df.columns:
            # 计算成交量变化
            if 'sample_id' in df.columns:
                df['_vol_change'] = df.groupby('sample_id')['vol'].transform(
                    lambda x: x.pct_change()
                )
            else:
                df['_vol_change'] = df['vol'].pct_change()
            df['volume_price_divergence'] = (
                np.sign(df['pct_chg']) != np.sign(df['_vol_change'])
            ).astype(int)
...
def process_file(input_file: Path) -> bool:
    ...
    # 检查是否已有交互特征
    interaction_cols = [
        'breakout_with_volume', 'momentum_market_interaction', 'rsi_kdj_divergence',
        'trend_consistency', 'volume_price_divergence', 'breakout_rsi_interaction',
        'relative_volatility', 'resonance_volume_confirm'
    ]
