...
class TestEnsurer:
    ...
    def get_modified_files(self) -> List[Path]:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "diff", "--name-only", "--cached"], cwd=self.project_root, capture_output=True, text=True
            )
    ...
    def check_all_modified(self, strict: bool = False) -> Dict[str, List[Path]]:
        ...
        return {"missing": missing_tests, "exists": existing_tests}
...
示例:
  ...
  python scripts/ensure_tests.py --generate
    ...
    parser.add_argument("--strict", "-s", action="store_true", help="严格模式：要求测试覆盖率>=80%")
    parser.add_argument("--generate", "-g", action="store_true", help="自动生成缺失的测试模板")
