...
def predict_top50(predict_date: str, enable_fundamental_screening: bool = False, screening_mode: str = "post_top50"):
    ...
    """
    log.info("=" * 80)
    log.info(f"v2.8.0集成模型预测 - Top50 - {predict_date}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="v2.8.0集成模型预测 - Top50")
    ...
    parser.add_argument(
        ...
        help=(
            "筛选模式: pre_all(预测前对全部筛选，慢), "
            "post_top50(预测后对Top50筛选，快，推荐), "
            "post_top100(预测后对Top100筛选，较快)"
        ),
    )
