...
def evaluate_top10(predict_file, eval_date):
    ...
    log.info(f"\n📋 Top10 详细结果:")
    log.info("-" * 100)
    log.info(f"{'排名':<4} {'代码':<12} {'名称':<10} {'综合评分':<10} {'校准概率':<10} {'预期收益':<10} {'预测价':<8} {'评估价':<8} {'收益率':<10}")
    log.info("-" * 100)
...
def compare_with_v230(predict_date, eval_date):
    ...
    # 加载v2.3.0的评估结果
    v230_file = PROJECT_ROOT / 'data' / 'prediction' / 'evaluation' / f'v2.3.0_top100_20251212.csv'
    ...
    # 加载v2.3.1的评估结果
    v231_file = PROJECT_ROOT / 'data' / 'prediction' / 'evaluation' / f'v2.3.1_top10_eval_{predict_date}_to_{eval_date}.csv'
