...
用法示例：
  ...
  python predict_v232_unified.py --date 20260122 --mainboard --fundamental --right-side
    ...
    parser.add_argument('--rsi-filter', type=float, nargs=2, metavar=('MIN', 'MAX'),
                       help='RSI区间筛选，如 --rsi-filter 40 70（与--right-side互斥）')
    
    # 板块配置
    parser.add_argument('--hot-sectors', type=str, default='',
                       help='自定义热门板块，逗号分隔，如：特高压,电力,机器人')
    ...
    log.info("="*80)
    log.info(f"执行流程：")
    log.info(f"  1. 全市场评分（模型对所有股票打分）")
    log.info(f"  2. 评分后筛选（在排名基础上应用筛选条件）")
    log.info(f"\n筛选条件（评分后应用）：")
    if args.mainboard:
        log.info(f"  - 仅沪深主板（过滤创业板、科创板、北交所）")
    if args.fundamental:
        log.info(f"  - 基本面筛选（市值10-100亿，ROE>5%等）")
    if args.right_side:
        log.info(f"  - 右侧模式（板块加成，轻度追高惩罚，不惩罚RSI）")
    if args.rsi_filter:
    ...
    valid = stock_list[
        ...
        ~stock_list['ts_code'].str.startswith('8') &      # 北交所
        ~stock_list['ts_code'].str.startswith('4') &      # 北交所
        ~stock_list['ts_code'].str.startswith('920')      # 北交所
    ].copy()
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for idx, row in valid.iterrows():
            future = executor.submit(
                ...
                feature_names, booster, calibrator, args.right_side
            )
    ...
    # 右侧模式：计算板块加成
    if args.right_side:
        df_all['sector_boost'] = df_all['industry'].apply(
            lambda x: get_sector_boost(x, hot_sectors)[0]
        )
        df_all['is_hot_sector'] = df_all['industry'].apply(
    ...
    if args.rsi_filter:
        ...
        df_rsi_filtered = df_top_n[
            (df_top_n['rsi_6'] >= rsi_min) & (df_top_n['rsi_6'] <= rsi_max)
        ].copy()
    ...
    if args.mainboard or args.fundamental:
        log.info(f"\n💡 筛选效果:")
        log.info(f"  全市场Top10中，筛选后保留: {len(df_all.head(10)[df_all.head(10)['ts_code'].isin(df_results['ts_code'])])}/10")
        log.info(f"  全市场Top50中，筛选后保留: {len(df_all.head(50)[df_all.head(50)['ts_code'].isin(df_results['ts_code'])])}/50")
