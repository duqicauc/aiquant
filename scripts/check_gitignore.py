...
def check_ignored_files(patterns: List[str]) -> List[Tuple[str, str]]:
    ...
    for pattern in patterns:
        ...
        for file_path in files:
            ...
            # 检查文件是否被忽略
            result = subprocess.run(["git", "check-ignore", "-v", str(file_path)], capture_output=True, text=True)
...
def check_untracked_code_files() -> List[str]:
    ...
    result = subprocess.run(
        ["git", "status", "--porcelain", "--untracked-files=all"], capture_output=True, text=True, cwd=git_root
    )
