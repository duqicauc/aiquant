...
def strategy_intersection(date, top_n=100, output_top=20):
    ...
    result = result.merge(
        df_240_inter[["ts_code", "calibrated_probability", "return_34d"]].rename(
            columns={"calibrated_probability": "v240_prob", "return_34d": "v240_return_34d"}
        ),
        on="ts_code",
    )
...
def strategy_signal_verification(date, watchlist_file=None, top_n=50, prob_threshold=0.7):
    ...
    result = result.merge(
        df_240_triggered[["ts_code", "calibrated_probability"]].rename(columns={"calibrated_probability": "v240_prob"}),
        on="ts_code",
    )
    result = result.merge(
        df_232_triggered[["ts_code", "calibrated_probability"]].rename(columns={"calibrated_probability": "v232_prob"}),
        on="ts_code",
    )
