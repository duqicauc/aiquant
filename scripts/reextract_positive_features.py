...
def main():
    ...
    df_features[numeric_cols] = df_features.groupby("sample_id")[numeric_cols].transform(lambda x: x.ffill().bfill())
    ...
    stats = {
        ...
        "days_per_sample": (
            len(df_features) // df_features["sample_id"].nunique() if df_features["sample_id"].nunique() > 0 else 0
        ),
    }
