...
def get_feature_columns(df, feature_cols):
    ...
    exclude_cols = ['ts_code', 'name', 't1_date', 't2_date', 'sample_id', 'label', 
                    ...
                    'days_to_t1', 'breakout_high_10d', 'breakout_high_20d', 'breakout_ma10', 
...
def save_ensemble_model(models, weights, feature_names, metrics):
    ...
    metadata = {
        ...
        'description': 'v2.7.0集成模型 - XGBoost+LightGBM+CatBoost加权投票',
        'metrics': metrics
    }
...
def main():
    ...
    log.info(f"{'Ensemble':<15} {metrics['auc']:>10.4f} {metrics['precision']:>12.4f} {metrics['recall']:>10.4f} {metrics['f1']:>10.4f}")
