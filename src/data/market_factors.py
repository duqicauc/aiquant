...
class MarketFactors:
    ...
    def get_index_data(
        self, index_code: str = "000001.SH", start_date: str = None, end_date: str = None
    ) -> pd.DataFrame:
        """
        获取指数日线数据
    ...
    def calculate_market_return(self, index_data: pd.DataFrame, window: int = 34) -> pd.DataFrame:
        """
        计算市场N日涨跌幅
        ...
        # 计算N日涨跌幅
        df[f"market_return_{window}d"] = (df["close"] / df["close"].shift(window) - 1) * 100

        # 计算N日波动率
        df[f"market_volatility_{window}d"] = df["pct_chg"].rolling(window=window).std()
    ...
    def calculate_relative_strength(
        self, stock_data: pd.DataFrame, market_data: pd.DataFrame, window: int = 34
    ) -> pd.DataFrame:
        """
        计算个股相对大盘的强度
        ...
        # 2. N日累计超额收益
        df["stock_return_nd"] = (df["close"] / df["close"].shift(window) - 1) * 100
        df["market_return_nd"] = (df["market_close"] / df["market_close"].shift(window) - 1) * 100
        ...
        df["relative_strength"] = np.where(
            ...
            np.where(df["market_return_nd"] < 0, -df["stock_return_nd"] / df["market_return_nd"], 0),
        )
    ...
    def calculate_historical_strength(
        self, stock_data: pd.DataFrame, lookback_window: int = 250, compare_window: int = 34  # 约1年
    ) -> pd.DataFrame:
        """
        计算个股相对历史均值的强度
        ...
        # 1. 价格相对历史均值位置
        df["price_vs_hist_mean"] = (df["close"] / df["close"].rolling(window=lookback_window).mean() - 1) * 100

        # 2. 价格相对历史高点位置
        df["price_vs_hist_high"] = (df["close"] / df["close"].rolling(window=lookback_window).max() - 1) * 100

        # 3. 价格相对历史低点位置
        df["price_vs_hist_low"] = (df["close"] / df["close"].rolling(window=lookback_window).min() - 1) * 100

        # 4. 成交量相对历史均值
        if "vol" in df.columns:
            df["volume_vs_hist_mean"] = df["vol"] / df["vol"].rolling(window=lookback_window).mean()
        ...
        # 计算N日涨幅
        df["return_nd"] = (df["close"] / df["close"].shift(compare_window) - 1) * 100

        # 当前N日涨幅在历史中的百分位
        df["return_percentile"] = rolling_percentile(df["return_nd"], lookback_window)
    ...
    def get_market_sentiment(self, trade_date: str) -> Dict:
        ...
        try:
            ...
            if df_limit is not None and not df_limit.empty:
                ...
                sentiment["limit_ratio"] = up_limit / (up_limit + down_limit) if (up_limit + down_limit) > 0 else 0.5
        ...
        try:
            ...
            if df_margin is not None and not df_margin.empty:
                ...
                sentiment["margin_ratio"] = (
                    total_rzye / (total_rzye + total_rqye) if (total_rzye + total_rqye) > 0 else 0.5
                )
    ...
    def enrich_stock_data_with_market_factors(
        self, stock_data: pd.DataFrame, index_code: str = "000001.SH", window: int = 34
    ) -> pd.DataFrame:
        """
        为个股数据添加市场因子
        ...
        market_data = self.get_index_data(
            index_code=index_code, start_date=extended_start.strftime("%Y%m%d"), end_date=end_date.strftime("%Y%m%d")
        )
        ...
        # 合并市场趋势数据
        market_cols = ["trade_date", f"market_return_{window}d", f"market_volatility_{window}d", "market_trend"]
        market_subset = market_data[[c for c in market_cols if c in market_data.columns]].copy()
...
def calculate_market_factors_for_samples(samples_df: pd.DataFrame, fetcher, window: int = 34) -> pd.DataFrame:
    """
    为样本数据批量计算市场因子
    ...
    market_data = mf.get_index_data(
        index_code="000001.SH", start_date=extended_start.strftime("%Y%m%d"), end_date=end_date.strftime("%Y%m%d")
    )
    ...
    # 准备合并的市场数据
    market_cols = ["trade_date", "pct_chg", f"market_return_{window}d", f"market_volatility_{window}d", "market_trend"]
    market_subset = market_data[[c for c in market_cols if c in market_data.columns]].copy()
    market_subset.columns = [
        "trade_date" if c == "trade_date" else f"market_{c}" if c == "pct_chg" else c for c in market_subset.columns
    ]
