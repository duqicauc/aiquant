...
class CacheManager:
    ...
    def get(self, data_type: str, **params) -> Optional[pd.DataFrame]:
        """
        从缓存获取数据
    ...
    def set(self, data_type: str, data: pd.DataFrame, expire_days: int = None, **params):
        """
        存储数据到缓存
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, 0)
                """,
                    (cache_key, data_type, data_json, datetime.now().isoformat(), expires_at),
                )
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    type_stats[row[0]] = {"count": row[1], "hits": row[2] or 0}

                # 数据库大小
                db_size = os.path.getsize(self.db_path) / (1024 * 1024)  # MB

                return {"total_records": total_count, "by_type": type_stats, "database_size_mb": round(db_size, 2)}
    ...
    def get_data(self, ts_code: str, data_type: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
        获取股票数据（按股票代码和日期范围）
    ...
    def save_data(self, data: pd.DataFrame, data_type: str, ts_code: str):
        """
        保存股票数据（按股票代码和数据类型）
        ...
        if "trade_date" not in data.columns:
            log.warning("数据缺少 trade_date 列，无法保存")
            return
    ...
    def has_data(self, ts_code: str, data_type: str, start_date: str, end_date: str) -> bool:
        """
        检查是否有指定日期范围的数据
    ...
    def get_missing_dates(self, ts_code: str, data_type: str, start_date: str, end_date: str) -> Optional[tuple]:
        """
        获取缺失的日期范围
