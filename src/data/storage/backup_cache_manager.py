...
from .cache_manager import SUPPORTED_DATA_TYPES, CacheManager
...
class BackupCacheManager(CacheManager):
    ...
    def get_data_with_backup(
        self, ts_code: str, data_type: str, start_date: str, end_date: str
    ...
    def save_data_with_backup(self, df: pd.DataFrame, data_type: str, ts_code: str):
        """
        同时保存到SQLite和CSV
    ...
    def save_to_csv(self, df: pd.DataFrame, ts_code: str, data_type: str):
        """
        保存数据到CSV文件（增量更新）
        ...
        if csv_file.exists():
            try:
                ...
                if "trade_date" in df_merged.columns:
                    df_merged = df_merged.drop_duplicates(subset=["ts_code", "trade_date"], keep="last").sort_values(
                        "trade_date"
                    )
    ...
    def read_from_csv(self, ts_code: str, data_type: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        """
        从CSV文件读取数据
        ...
        try:
            ...
            # 过滤日期范围
            df_filtered = df[(df["trade_date"] >= start_date) & (df["trade_date"] <= end_date)].sort_values(
                "trade_date"
            )
