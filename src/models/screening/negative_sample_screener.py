...
class NegativeSampleScreener:
    ...
    def analyze_positive_features(self, positive_features_df: pd.DataFrame) -> Dict:
        """
        分析正样本的特征分布
        ...
        for sample_id in positive_features_df["sample_id"].unique():
            sample_data = positive_features_df[positive_features_df["sample_id"] == sample_id]
            ...
            if "pct_chg" in sample_data.columns:
                ...
                cumulative_return = ((1 + sample_data["pct_chg"] / 100).prod() - 1) * 100
            else:
                cumulative_return = 0
        ...
        return {"summary": stats_summary, "detail": df_stats}
    ...
    def screen_negative_samples(
        ...
        for idx, row in stock_list.iterrows():
            ...
            # 显示进度
            if (idx + 1) % 100 == 0:
                log.info(f"进度: {idx + 1}/{len(stock_list)} | " f"找到负样本: {len(negative_samples)}")
            ...
            try:
                ...
                if samples:
                    ...
                    log.success(f"✓ {ts_code} {name}: 找到 {len(samples)} 个负样本")
    ...
    def _screen_single_stock_negative(
        ...
        try:
            ...
            if not df_factor.empty:
                df = pd.merge(df, df_factor[["trade_date", "macd_dif"]], on="trade_date", how="left")
        except Exception:
            pass
        ...
        for i in range(len(df) - 33):
            ...
            if "pct_chg" in window.columns:
                cumulative_return = ((1 + window["pct_chg"] / 100).prod() - 1) * 100
            else:
                continue

            # 检查是否满足条件
            if (
                volume_min <= volume_count <= volume_max
                and macd_min <= macd_turn_count <= macd_max
                and return_min <= cumulative_return <= return_max
            ):
    ...
    def extract_features(self, negative_samples_df: pd.DataFrame) -> pd.DataFrame:
        """
        提取负样本的特征数据（34天）
        ...
        for idx, sample in negative_samples_df.iterrows():
            ...
            try:
                ...
                df = df[(df["trade_date"] >= int(start_date)) & (df["trade_date"] <= int(end_date))]
                ...
                try:
                    ...
                    if not df_factor.empty:
                        # 确保trade_date是整数类型
                        if df_factor["trade_date"].dtype == "object" or "datetime" in str(
                            df_factor["trade_date"].dtype
                        ):
                            df_factor["trade_date"] = (
                                pd.to_datetime(df_factor["trade_date"]).dt.strftime("%Y%m%d").astype(int)
                            )

                        # 只保留在start_date和end_date之间的数据
                        df_factor = df_factor[
                            (df_factor["trade_date"] >= int(start_date)) & (df_factor["trade_date"] <= int(end_date))
                        ]
                        df = pd.merge(
                            df,
