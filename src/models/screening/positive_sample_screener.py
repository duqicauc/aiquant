...
class PositiveSampleScreener:
    ...
    def screen_all_stocks(self, start_date: str = "20000101", end_date: str = None) -> pd.DataFrame:
        """
        筛选所有股票的正样本
        ...
        for idx, row in stock_list.iterrows():
            ...
            try:
                # 筛选该股票的正样本
                samples = self._screen_single_stock(ts_code, name, list_date, start_date, end_date)
    ...
    def _screen_single_stock(
        self, ts_code: str, name: str, list_date: pd.Timestamp, start_date: str, end_date: str
    ) -> List[Dict]:
        """
        筛选单只股票的正样本
        ...
        try:
            df_weekly = self.dm.get_weekly_data(ts_code, start_date, end_date, adjust="qfq")  # 前复权
        except Exception as e:
            log.warning(f"{ts_code} 周线数据获取失败，尝试本地转换: {e}")
            # 如果周线API失败，回退到日线转换方式
            df_daily = self.dm.get_daily_data(ts_code, start_date, end_date, adjust="qfq")
        ...
        for i in range(len(df_weekly) - 2):
            ...
            # 检查是否符合条件
            result = self._check_three_week_pattern(three_weeks, ts_code, name, list_date)
    ...
    def _calculate_pre_t1_return(self, ts_code: str, t1_date: pd.Timestamp, lookback_days: int = 34) -> float:
        """
        计算T1前N天的涨幅
    ...
    def _calculate_pre_t1_volatility(self, ts_code: str, t1_date: pd.Timestamp, lookback_days: int = 34) -> float:
        """
    ...
    def extract_features(self, samples_df: pd.DataFrame, lookback_days: int = 70) -> pd.DataFrame:
        """
        提取样本的特征数据（T1前N天）
            ...
            if not df_daily.empty:
                for col in missing_ohlcv:
                    if col in df_daily.columns:
                        # 按trade_date对齐
                        df = pd.merge(
                            df, df_daily[["trade_date", col]], on="trade_date", how="left", suffixes=("", "_daily")
                        )
