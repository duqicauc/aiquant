...
class FundamentalScreener:
    ...
    def get_latest_financial_data(self, ts_code: str, trade_date: str) -> Optional[Dict]:
        ...
        try:
            # 1. 获取市值（从daily_basic）
            df_basic = self.dm.get_daily_basic(stock_code=ts_code, trade_date=trade_date)

            if df_basic.empty:
                # 尝试获取最近的数据
                end_date = trade_date
                start_date = (datetime.strptime(trade_date, "%Y%m%d") - timedelta(days=30)).strftime("%Y%m%d")
                df_basic = self.dm.get_daily_basic(stock_code=ts_code, start_date=start_date, end_date=end_date)
    ...
    def _get_fina_indicator(self, ts_code: str, start_date: str, end_date: str) -> Optional[Dict]:
        ...
        try:
            ...
            df = fetcher.pro.fina_indicator(
                ts_code=ts_code, start_date=start_date, end_date=end_date, fields="ts_code,end_date,roe,roa"
            )
    ...
    def _get_income(self, ts_code: str, start_date: str, end_date: str) -> Optional[Dict]:
        ...
        try:
            ...
            df = fetcher.pro.income(
                ts_code=ts_code, start_date=start_date, end_date=end_date, fields="ts_code,end_date,revenue,n_income"
            )
    ...
    def screen_stocks(self, stocks_df: pd.DataFrame, trade_date: str) -> pd.DataFrame:
        ...
        log.info("筛选条件:")
        log.info(
            f"  市值范围: {self.config['market_cap_min']/10000:.0f}亿 - {self.config['market_cap_max']/10000:.0f}亿"
        )
    ...
    def filter_stocks(self, stocks_df: pd.DataFrame, trade_date: str) -> pd.DataFrame:
        ...
        filtered_df = screened_df[screened_df["fundamental_pass"]].copy()
