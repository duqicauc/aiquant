#!/usr/bin/env python3
"""
BouncePredictor — v3.1.0 超跌反弹模型预测器

预测流程：
1. 加载 Bounce 模型 + 校准器 + 特征名
2. 构建预测样本（全市场股票 + 预测日）
3. BounceFeatureExtractor 提取 64 天多行特征
...
class BouncePredictor:
    """v3.1.0 BounceScorer 预测器"""

    def __init__(self, model_dir: Optional[Path] = None):
        self.model_dir = Path(model_dir) if model_dir else MODEL_DIR
        self.model, self.calibrator, self.feature_names = self._load_artifacts()
        self.extractor = BounceFeatureExtractor(use_cache=True)
        log.info(f"BouncePredictor 初始化完成: {len(self.feature_names)} 维")

    def _load_artifacts(self):
        """加载模型、校准器、特征名"""
        version_dirs = sorted([d for d in self.model_dir.iterdir() if d.is_dir()], reverse=True)
        if not version_dirs:
            raise FileNotFoundError(f"Bounce 模型目录不存在: {self.model_dir}")
    ...
    def predict_date(
        ...
        """
        log.info(f"{'='*60}")
        log.info(f"BouncePredictor 预测: {date}")
        log.info(f"{'='*60}")
        ...
        # 2. 提取 40 天多行特征（v3.1.0重构: 64→40）
        df_features = self.extractor.extract_for_samples(
            samples_df, lookback_days=40, label=0
        )
        ...
        log.success(f"Bounce 预测完成: {len(result)} 只股票")
        return result
    ...
    def predict_range(
        ...
        log.info(f"Bounce 批量预测: {start_date} ~ {end_date}, {len(trade_dates)} 个交易日")
    ...
    def _get_stock_list(self, date: str) -> List[str]:
        ...
        for attempt in range(3):
            try:
                ...
                log.warning(f"获取股票基本信息失败 (尝试{attempt+1}/3): {e}")
            if attempt < 2:
                time.sleep(1)

        if stock_basic is None or stock_basic.empty:
            log.warning("未获取到股票基本信息")
            return []
        ...
        for attempt in range(3):
            try:
                ...
                log.warning(f"获取 {date} 股票列表失败 (尝试{attempt+1}/3): {e}")
            if attempt < 2:
                time.sleep(1)
